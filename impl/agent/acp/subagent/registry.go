...
type AgentConfig = assembly.AgentConfig
...
func (r *Registry) Register(cfg AgentConfig) error {
	...
	if cfg.Name == "" {
		return fmt.Errorf("impl/agent/acp/subagent: agent name is required")
	}
	if strings.TrimSpace(cfg.Command) == "" {
		return fmt.Errorf("impl/agent/acp/subagent: command is required for agent %q", cfg.Name)
	}
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Registry) Get(_ context.Context, name string) (delegation.Agent, error) {
	cfg, err := r.lookup(name)
	if err != nil {
		return delegation.Agent{}, err
	}
	return delegation.Agent{Name: cfg.Name, Description: cfg.Description}, nil
}

func (r *Registry) List(context.Context) ([]delegation.Agent, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	out := make([]delegation.Agent, 0, len(r.agents))
	for _, one := range r.agents {
		out = append(out, delegation.Agent{Name: one.Name, Description: one.Description})
	}
	return out, nil
}
...
func (r *Registry) lookup(name string) (AgentConfig, error) {
	if r == nil {
		return AgentConfig{}, fmt.Errorf("impl/agent/acp/subagent: registry is unavailable")
	}
	...
	if !ok {
		return AgentConfig{}, fmt.Errorf("impl/agent/acp/subagent: agent %q not found", name)
	}
