#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into ~/.caelis/skills."""
...
def _skills_dir() -> str:
    configured = os.environ.get("CAELIS_SKILLS_DIR")
    if configured:
        return os.path.expanduser(configured)
    caelis_home = os.environ.get("CAELIS_HOME", os.path.expanduser("~/.caelis"))
    return os.path.join(os.path.expanduser(caelis_home), "skills")


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "caelis")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "caelis-skill-install")
