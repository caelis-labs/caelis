...
func runProviderJSONOutputE2E(t *testing.T, provider string, defaultModel string) {
	...
	for event, err := range spec.LLM.Generate(ctx, &model.Request{
		Instructions: []model.Part{
			model.NewTextPart("Return only valid JSON. Do not include markdown or prose."),
		},
		Messages: []model.Message{
			model.NewTextMessage(model.RoleUser, `Return exactly this JSON object shape with status set to "ok" and provider set to the provider name you are serving: {"status":"ok","provider":"..."}`),
		},
