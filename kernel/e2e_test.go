...
func TestGatewayProviderLiveTurnAndReplayE2E(t *testing.T) {
	...
	for env := range result.Handle.Events() {
		...
		switch {
		case payload.Role == kernel.NarrativeRoleUser:
			sawUser = true
		case payload.Role == kernel.NarrativeRoleAssistant &&
			payload.Visibility == string(session.VisibilityUIOnly) &&
			(payload.UpdateType == string(session.ProtocolUpdateTypeAgentMessage) ||
				payload.UpdateType == string(session.ProtocolUpdateTypeAgentThought)):
			sawChunk = true
		case payload.Role == kernel.NarrativeRoleAssistant &&
			payload.Visibility == string(session.VisibilityCanonical):
	...
	replayed, err := gw.ReplayEvents(ctx, kernel.ReplayEventsRequest{
		SessionRef: session.SessionRef,
	})
	if err != nil {
...
func newGatewayProviderStack(t *testing.T, model model.LLM) (*kernel.Gateway, session.Session) {
	return newGatewayProviderStackWithMetadata(t, model, nil)
}

func newGatewayProviderStackWithMetadata(t *testing.T, model model.LLM, metadata map[string]any) (*kernel.Gateway, session.Session) {
	t.Helper()
