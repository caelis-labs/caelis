...
func (b *FeedBroker) prepareBackfillPlan(
	...
	plan := &feedBackfillPlan{
		...
		startAcceptID: startAcceptID, ring: selectedRing, freshReplay: strings.TrimSpace(req.Cursor) == "",
	}
	if checkpoint.ThroughSeq > 0 {
		if b.reader == nil {
...
func (p *feedBackfillPlan) run(subscriber *feedSubscription) error {
	...
	ring := p.ring
	var selectedDurable map[freshReplayContentKey]freshReplaySelectedSource
	if p.freshReplay {
		complete := freshReplayCompleteSources(ring)
		selectedDurable = make(map[freshReplayContentKey]freshReplaySelectedSource)
		if p.throughSeq > 0 {
			materializations := newDurableEnvelopeIterator(p)
			for {
				envelope, ok, err := materializations.next()
				if err != nil {
				...
				addFreshReplayCompleteSource(complete, envelope)
				addFreshReplayDurableCompleteSource(selectedDurable, envelope)
			}
		}
		ring = selectFreshReplaySources(ring, complete, p.throughSeq)
	}

	iterator := newDurableEnvelopeIterator(p)
	nextDurable := func() (eventstream.Envelope, bool, error) {
		for {
			envelope, ok, err := iterator.next()
			if err != nil || !ok || !p.freshReplay {
				return envelope, ok, err
			}
			envelope, deliver := selectFreshReplayDurableEnvelope(envelope, selectedDurable)
			if deliver {
				return envelope, true, nil
			}
		}
	}
	current, hasCurrent, err := nextDurable()
	if err != nil {
		return p.gapError(subscriber, err)
	}
