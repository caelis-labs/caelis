...
func (s *FileOperationStore) AcquireExecution(ctx context.Context, intent OperationIntent) (OperationExecutionLease, error) {
	if s == nil || strings.TrimSpace(s.root) == "" || s.root == "." {
		return nil, errors.New("controlclient: operation store root is required")
	}
	if ctx == nil {
		ctx = context.Background()
	}
	...
	// The product has one Control Host. Exact duplicate execution is therefore a
	// process-local concern, while restart recovery is proved by the durable
	// operation intent plus the domain receipt. Do not extend this gate into a
	// cross-process lease that can outlive an interactive or external effect.
	key := "file:" + s.root + ":" + operationKey(intent.PrincipalID, intent.OperationID)
	return acquireOperationExecutionGate(ctx, key)
}
...
func (s *FileOperationStore) Complete(ctx context.Context, intent OperationIntent, result CommandResult) (OperationRecord, error) {
	...
	err := s.withRootLock(ctx, func() error {
		...
		if persisted.Result != nil {
			record = persisted
			if !sameCommandResult(*persisted.Result, result) {
				return ErrOperationConflict
			}
			return nil
		}
