...
func TestFeedBrokerForwardsTerminalDeltasWithoutContentReconciliation(t *testing.T) {
	broker, _ := newTestFeedBroker(t, nil, FeedBrokerConfig{RingEvents: 8, SubscriberQueue: 8})
	subscription, err := broker.SubscribeFromNow(context.Background())
	if err != nil {
		t.Fatal(err)
	...
	for index, text := range []string{"步骤 1: 正在处理...\n", "步骤 2: 正在处理...\n", "步骤 2: 正在处理...\n"} {
		if err := broker.Publish(terminalEnvelopeAtCursor(text, int64(index+1))); err != nil {
			t.Fatal(err)
		}
		got := receiveEnvelopes(t, subscription.Events(), 1)[0]
		if output, ok := terminalEnvelopeOutput(got); !ok || output != text {
			t.Fatalf("terminal delta %d = %q, %v; want exact producer bytes %q", index, output, ok, text)
		}
	}
}

func TestFeedBrokerFreshReplaySelectsCanonicalNarrativeInsteadOfRetainedDeltas(t *testing.T) {
	broker, _ := newTestFeedBroker(t, nil, FeedBrokerConfig{RingEvents: 8, SubscriberQueue: 8})
	for _, text := range []string{"CAE", "LIS"} {
	...
	if _, open := <-result.Subscription.Backfill(); open {
		t.Fatal("fresh replay retained live narrative deltas beside the canonical message")
	}
}

func TestFeedBrokerFreshReplaySelectsCanonicalTerminalInsteadOfRetainedDeltas(t *testing.T) {
	broker, _ := newTestFeedBroker(t, nil, FeedBrokerConfig{RingEvents: 8, SubscriberQueue: 8})
	for index, text := range []string{"line 1\n", "line 2\n"} {
		if err := broker.Publish(terminalEnvelopeAtCursor(text, int64(index+1))); err != nil {
			t.Fatal(err)
		}
	}
	...
	defer result.Subscription.Close()
	got := receiveEnvelopes(t, result.Subscription.Backfill(), 1)
	if output, ok := terminalEnvelopeOutput(got[0]); !ok || output != "line 1\nline 2\n" || !isDurableFeedEnvelope(got[0]) {
		t.Fatalf("fresh terminal replay = %#v, want one durable complete output", got)
	}
	if _, open := <-result.Subscription.Backfill(); open {
		t.Fatal("fresh replay retained terminal deltas beside the canonical output")
	}
}

func TestFeedBrokerFreshReplayRetainsTerminalDeltasBesideStateOnlyFinal(t *testing.T) {
	broker, _ := newTestFeedBroker(t, nil, FeedBrokerConfig{RingEvents: 8, SubscriberQueue: 8})
	for index, text := range []string{"line 1\n", "line 2\n"} {
		if err := broker.Publish(terminalEnvelopeAtCursor(text, int64(index+1))); err != nil {
			t.Fatal(err)
	...
	defer result.Subscription.Close()
	got := receiveEnvelopes(t, result.Subscription.Backfill(), 3)
	for index, want := range []string{"line 1\n", "line 2\n"} {
		if output, ok := terminalEnvelopeOutput(got[index]); !ok || output != want {
...
func TestFeedBrokerFreshReplaySelectsStoredCanonicalNarrativeOverRetainedDeltas(t *testing.T) {
	final := model.NewTextMessage(model.RoleAssistant, "CAELIS")
	reader := &checkpointPageReader{events: []*session.Event{{
		ID: "event-1", SessionID: "session-1", Seq: 1,
	...
	// Model the live-supplement production cut: storage has advanced through the
	// canonical final while the captured ring contains only retained deltas.
	broker.mu.Lock()
	broker.scannedSeq = 1
	broker.mu.Unlock()
	...
	if _, open := <-result.Subscription.Backfill(); open {
		t.Fatal("fresh replay retained live deltas beside the stored canonical narrative")
	}
}

func TestFeedBrokerFreshReplayLetsStoredTerminalOutputWinAfterRingEviction(t *testing.T) {
	reader := &checkpointPageReader{events: []*session.Event{{
		ID: "command-final", SessionID: "session-1", Seq: 2,
	...
	broker.mu.Lock()
	broker.scannedSeq = 2
	broker.latestDurable = eventstream.DurableFeedPosition{Seq: 1}
	broker.mu.Unlock()
	for index, text := range []string{"line 1\n", "line 2\n", "line 3\n"} {
		envelope := terminalEnvelopeAtCursor(text, int64(index+1))
		envelope.TurnID = "turn-1"
		envelope.Scope = eventstream.ScopeMain
		envelope.ScopeID = "turn-1"
		if err := broker.Publish(envelope); err != nil {
			t.Fatal(err)
		}
	}
	...
	got := receiveEnvelopes(t, result.Subscription.Backfill(), 1)
	if output, ok := terminalEnvelopeOutput(got[0]); !ok || output != "line 1\nline 2\nline 3\n" || !isDurableFeedEnvelope(got[0]) {
		t.Fatalf("fresh terminal replay = %#v, want complete stored output", got)
	}
	if _, open := <-result.Subscription.Backfill(); open {
		t.Fatal("fresh terminal replay retained an evicted delta suffix or state-only ring final")
	}
}
...
func TestFeedBrokerFreshReplaySelectsFinalCommandTaskMaterialization(t *testing.T) {
	...
	toolResult := func(
		...
		return &session.Event{
			ID: "event-" + callID, SessionID: "session-1", Seq: seq,
			Type: session.EventTypeToolResult, Visibility: session.VisibilityCanonical,
			Scope: &session.EventScope{TurnID: "turn-1"}, Meta: meta,
			Tool: &session.EventTool{ID: callID, Name: name, Status: status, Input: input, Output: output},
			Protocol: &session.EventProtocol{Method: session.ProtocolMethodSessionUpdate, Update: &session.ProtocolUpdate{
				SessionUpdate: string(session.ProtocolUpdateTypeToolUpdate), ToolCallID: callID,
				Kind: name, Title: name, Status: status, RawInput: input, RawOutput: output, Meta: meta,
			}},
		}
	}
	reader := &checkpointPageReader{events: []*session.Event{
	...
	var outputs []string
	for _, envelope := range got {
		if output, ok := terminalEnvelopeOutput(envelope); ok {
			outputs = append(outputs, output)
		}
	}
	if len(outputs) != 1 || outputs[0] != "CHUNK_A_B\nCHUNK_C\n" {
		t.Fatalf("fresh terminal outputs = %#v from %#v, want only the final complete Task materialization", outputs, got)
	}
	if got[2].ParentTool == nil || got[2].ParentTool.ToolCallID != "command-call-1" || got[2].ParentTool.ToolName != "RunCommand" {
		t.Fatalf("final Task parent relation = %#v, want typed RunCommand/command-call-1", got[2].ParentTool)
	}
	if _, open := <-result.Subscription.Backfill(); open {
