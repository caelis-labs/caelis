...
// DefaultProfileError reports that an unbound system Agent cannot resolve an
// eligible Control-owned default profile.
...
func (e *DefaultProfileError) Error() string {
	...
	if e.ProfileID == "" {
		return fmt.Sprintf("control/placement: %q has no default profile", e.Handle)
	}
	return fmt.Sprintf(
...
func defaultSystemProfile(profiles modelprofile.Configuration, handle agentbinding.Handle) (modelprofile.ModelProfile, error) {
	...
	if !agentbinding.SupportsProfile(handle, profile) {
		return modelprofile.ModelProfile{}, &DefaultProfileError{
			Handle: handle, ProfileID: profile.ID, Reason: fmt.Sprintf("cannot use its %s backend", profile.Kind()),
		}
	}
	return profile, nil
}
