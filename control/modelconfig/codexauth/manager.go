...
// The transport is restricted to the Codex backend allowlist, refreshes the
// saved access token with a 60-second expiry skew, and silently refreshes and
// retries one replayable request when the backend rejects that token with 401.
