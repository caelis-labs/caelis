...
func TestAuthenticatedClientRefreshesAndRetriesUnauthorizedOnce(t *testing.T) {
	t.Parallel()
	...
	base := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		if call == 1 {
			...
			return &http.Response{StatusCode: http.StatusUnauthorized, Header: make(http.Header), Body: trackedResponseBody{Reader: strings.NewReader("unauthorized"), closed: &firstBodyClosed}, Request: request}, nil
		}
	...
	request, _ := http.NewRequest(http.MethodPost, "https://chatgpt.com/backend-api/codex/responses", strings.NewReader(requestPayload))
	request.Header.Set("Authorization", "caller-value")
	response, err := client.Do(request)
	if err != nil {
		t.Fatal(err)
	...
	_ = response.Body.Close()
	wantAuthorization := "Bearer " + jwtForTest("account", nil, now.Add(time.Hour))
	if response.StatusCode != http.StatusOK || backendCalls.Load() != 2 || refreshCalls.Load() != 1 || secondAuthorization != wantAuthorization {
		t.Fatalf("request = status %d backend=%d refresh=%d auth=%q, want one silent refresh and retry with %q", response.StatusCode, backendCalls.Load(), refreshCalls.Load(), secondAuthorization, wantAuthorization)
	}
	...
	if request.Header.Get("Authorization") != "caller-value" {
...
func TestConcurrentUnauthorizedRequestsShareSilentRefresh(t *testing.T) {
	...
	base := &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		return &http.Response{StatusCode: status, Header: make(http.Header), Body: io.NopCloser(strings.NewReader(http.StatusText(status))), Request: request}, nil
	})}
	client, refreshCalls := newRevokedTestAuthenticatedClient(t, now, base)
...
func TestAuthenticatedClientDoesNotReplayNonReplayableUnauthorizedRequest(t *testing.T) {
	...
	client, refreshCalls := newRevokedTestAuthenticatedClient(t, now, base)
	request, _ := http.NewRequest(http.MethodPost, "https://chatgpt.com/backend-api/codex/responses", io.NopCloser(strings.NewReader("{}")))
