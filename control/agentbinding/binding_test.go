...
func TestReviewerAcceptsACPWhileGuardianRejectsItBeforeMutation(t *testing.T) {
	current := Configuration{Bindings: []Binding{{Handle: HandleGuardian, ProfileID: "provider:model", Effort: "high"}}}
	withReviewer, err := Bind(current, Binding{Handle: HandleReviewer, ProfileID: "acp:claude:opus", Effort: "xhigh"}, testProfiles())
	if err != nil {
		t.Fatalf("Bind(ACP reviewer) error = %v", err)
	}
	if reviewer, ok := Lookup(withReviewer, HandleReviewer); !ok || reviewer.ProfileID != "acp:claude:opus" {
		t.Fatalf("Reviewer binding = %#v, %v", reviewer, ok)
	}

	_, err = Bind(withReviewer, Binding{Handle: HandleGuardian, ProfileID: "acp:claude:opus", Effort: "xhigh"}, testProfiles())
