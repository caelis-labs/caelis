...
// Mutation describes one Plugin configuration change. The host owns candidate
// validation and revision-CAS persistence; Control owns the state transition.
// AfterCommit runs synchronously after a committed write. Its error reports a
// committed partial success and must not trigger rollback.
...
// Plugin state persistence and candidate validation, and a read-only live MCP
// status probe. UpdatePluginState must invoke Apply and
