...
func (s Service) fetchMarketplaceRoot(ctx context.Context, source string) (root string, repoURL string, err error) {
	...
	if absPath, absErr := filepath.Abs(source); absErr == nil {
		if fi, statErr := os.Stat(absPath); statErr == nil && fi.IsDir() {
			...
			// Local marketplace roots are caller-owned and are not rewritten.
			return absPath, "", nil
		}
	}
	...
	parent := filepath.Join(storeDir, "plugins", "marketplaces", marketplaceCacheDirName(source))
	root, err = cloneGitRepoImmutable(ctx, repoURL, "", parent, "")
	if err != nil {
		return "", "", fmt.Errorf("plugin service: fetch marketplace: %w", err)
	}
...
func (s Service) cloneMarketplacePluginSource(ctx context.Context, repoURL string, ref string, sha string, subpath string, cacheKey pluginInstallCacheKey) (string, error) {
	...
	// Parent is the stable target namespace. Content is published under a
	// content-addressed child and never overwrites an existing published root.
	parent := filepath.Join(storeDir, "plugins", "installed", pluginInstallCacheDirName(cacheKey))
	root, err := cloneGitRepoImmutable(ctx, repoURL, ref, parent, sha)
	if err != nil {
		return "", err
	}
