...
func TestResolveMarketplaceGitURLAcceptsOfficialAlias(t *testing.T) {
	t.Parallel()

	got, err := resolveMarketplaceGitURL("claude-plugins-official")
	if err != nil {
		t.Fatalf("resolveMarketplaceGitURL() error = %v", err)
	}
	if got != "https://github.com/anthropics/claude-plugins-official.git" {
		t.Fatalf("resolveMarketplaceGitURL() = %q, want official marketplace clone url", got)
	}
}
