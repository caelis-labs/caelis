...
func (s *StateService) reconnect(
	...
	if prepare && s.config.PrepareReconnect != nil {
		if err := s.config.PrepareReconnect(ctx, ref); err != nil {
			return ReconnectResult{}, err
		}
	}
	var releaseObservation func()
	if prepare && s.config.RetainObservation != nil {
		var retainErr error
		releaseObservation, retainErr = s.config.RetainObservation(ref)
		if retainErr != nil {
			return ReconnectResult{}, retainErr
		}
	}
	observationTransferred := false
	defer func() {
		if !observationTransferred && releaseObservation != nil {
			releaseObservation()
		}
	}()
	...
	if releaseObservation != nil {
		subscribed.Subscription = &observationSubscription{
			FeedSubscription: subscribed.Subscription,
...
// observationSubscription couples the in-memory Session observation reference
// to the exact reconnect continuation that represents it. Closing is
// idempotent even when transport teardown and a caller race.
...
func (s *observationSubscription) Close() error {
	if s == nil {
		return nil
	}
	var err error
	s.once.Do(func() {
		if s.FeedSubscription != nil {
			err = s.FeedSubscription.Close()
		}
		if s.release != nil {
			s.release()
		}
