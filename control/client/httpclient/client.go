...
type Config struct {
	...
	EventBuffer   int
	MaxEventBytes int
	Compatibility controlclient.CompatibilityPolicy
}
...
func (c *Client) Health(ctx context.Context) (controlclient.HostStatus, error) {
...
func (c *Client) Readiness(ctx context.Context) (controlclient.HostStatus, error) {
...
func (c *Client) ShutdownHost(ctx context.Context) (controlclient.HostStatus, error) {
	response, err := c.do(ctx, http.MethodPost, "/host/shutdown", nil, nil, nil)
	if err != nil {
		return controlclient.HostStatus{}, err
	}
	defer response.Body.Close()
	raw, err := readRemoteResponse(response)
	if err != nil {
	...
	var status controlclient.HostStatus
	if err := wirev1.Unmarshal(raw, &status); err != nil {
		return controlclient.HostStatus{}, fmt.Errorf("control http client: decode Host shutdown response: %w", err)
	}
	return status, nil
}

func (c *Client) hostStatus(ctx context.Context, path string, accepted ...int) (controlclient.HostStatus, error) {
	if c == nil || c.baseURL == nil {
		return controlclient.HostStatus{}, errors.New("control http client: client is unavailable")
	}
	endpoint := *c.baseURL
	endpoint.Path = path
	endpoint.RawPath = ""
	...
	if err != nil {
		return controlclient.HostStatus{}, fmt.Errorf("control http client: build Host status request: %w", err)
	}
	response, err := c.httpClient.Do(request)
	if err != nil {
		return controlclient.HostStatus{}, fmt.Errorf("control http client: Host status request: %w", err)
	}
	defer response.Body.Close()
	allowed := false
	...
	if !allowed {
		...
		if readErr != nil {
			return controlclient.HostStatus{}, readErr
		}
		return controlclient.HostStatus{}, fmt.Errorf("control http client: unexpected Host status HTTP %d: %s", response.StatusCode, strings.TrimSpace(string(raw)))
	}
	raw, err := io.ReadAll(io.LimitReader(response.Body, maxRemoteResponseBody+1))
	if err != nil {
		return controlclient.HostStatus{}, fmt.Errorf("control http client: read Host status: %w", err)
	}
	if len(raw) > maxRemoteResponseBody {
		return controlclient.HostStatus{}, errors.New("control http client: Host status response is too large")
	}
	var status controlclient.HostStatus
	if err := json.Unmarshal(raw, &status); err != nil {
		return controlclient.HostStatus{}, fmt.Errorf("control http client: decode Host status: %w", err)
	}
	return status, nil
}
...
func (c *Client) validateRemoteState(state controlclient.SessionState) error {
	...
	return policy.Accept(controlclient.ServerInfo{
