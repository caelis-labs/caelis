...
func TestNewRejectsInsecureRemoteOrigin(t *testing.T) {
	...
	if _, err := New(Config{
		BaseURL: "https://control.example.test", BearerToken: "secret", Compatibility: controlclient.CurrentCompatibility(),
	}); err != nil {
...
func TestRemoteStateValidationDoesNotRepeatHostCapabilityHandshake(t *testing.T) {
	client := &Client{compatibility: controlclient.CurrentCompatibility("required-host-capability")}
	if err := client.validateRemoteState(controlclient.SessionState{
...
func TestHostHealthAndReadinessUseRootLifecycleEndpoints(t *testing.T) {
	client, closeServer := newFixtureClient(t, func(writer http.ResponseWriter, request *http.Request) {
		...
		_, _ = fmt.Fprintf(writer, `{"server_id":%q,"instance_id":%q,"ready":false}`,
			controlclient.ServerIdentity, "11111111-1111-4111-8111-111111111111")
	})
	defer closeServer()
...
func TestStartParticipantPreservesTypedWriteContract(t *testing.T) {
	var started controlclient.StartParticipantRequest
	client, closeServer := newFixtureClient(t, func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
	...
	revision := uint64(math.MaxUint64)
	result, err := client.StartParticipant(context.Background(), controlclient.StartParticipantRequest{
		WriteBase: controlclient.WriteBase{
			OperationID:      "operation-remote-start",
			SessionID:        "session-1",
			ExpectedRevision: &revision,
...
func TestHostFocusedRequestsUseUnscopedRoutes(t *testing.T) {
	...
	client, closeServer := newFixtureClient(t, func(w http.ResponseWriter, r *http.Request) {
		...
		case 2:
			...
			var request controlclient.UseModelRequest
		...
		case 3:
			...
			var request controlclient.SandboxRequest
			decodeFixtureRequest(t, r, &request)
		...
		case 2:
			writeFixtureJSON(t, w, http.StatusOK, controlclient.CommandResult{
				OperationID: "model-use-1", Outcome: controlclient.OutcomeCommitted, Revision: 5,
			})
		case 3:
			writeFixtureJSON(t, w, http.StatusOK, controlclient.CommandResult{
				OperationID: "sandbox-reset-1", Outcome: controlclient.OutcomeCommitted, Revision: 4,
			})
		default:
			writeFixtureJSON(t, w, http.StatusOK, []controlclient.CompletionCandidate{})
		}
		index++
	})
	defer closeServer()
	if _, err := client.SessionStatus(context.Background(), controlclient.StatusRequest{Surface: "tui"}); err != nil {
		t.Fatal(err)
	}
	if _, err := client.CompleteSkill(context.Background(), controlclient.CompletionRequest{Surface: "tui"}); err != nil {
		t.Fatal(err)
	}
	modelRevision := uint64(4)
	if _, err := client.UseModel(context.Background(), controlclient.UseModelRequest{
		WriteBase: controlclient.WriteBase{OperationID: "model-use-1", ExpectedRevision: &modelRevision},
		Model:     "mimo",
	}); err != nil {
		t.Fatal(err)
	}
	expectedRevision := uint64(4)
	if _, err := client.ResetSandbox(context.Background(), controlclient.SandboxRequest{WriteBase: controlclient.WriteBase{
...
func TestFocusedMutationPreservesRemoteSessionClosed(t *testing.T) {
	...
	_, err := client.ConfigureSessionMode(context.Background(), controlclient.SessionModeRequest{
		WriteBase: controlclient.WriteBase{OperationID: "mode-1", SessionID: "session-1", ExpectedRevision: &revision, ExpectedControllerEpoch: "epoch-1"},
		Mode:      "manual",
	})
...
func newFixtureClientWithConfig(t *testing.T, config Config, handler http.HandlerFunc) (*Client, func()) {
	...
	if err := config.Compatibility.Validate(); err != nil {
		config.Compatibility = controlclient.CurrentCompatibility()
	}
