...
func TestUint64WireRoundTripAtJavaScriptBoundary(t *testing.T) {
	...
	for _, value := range values {
		...
		t.Run(strconv.FormatUint(value, 10), func(t *testing.T) {
			...
			sandboxJSON := mustMarshalWire(t, controlclient.SandboxRequest{WriteBase: controlclient.WriteBase{
			...
			var decodedSandbox controlclient.SandboxRequest
			...
			modelRequests := []struct {
			...
			}{
				{
					...
					request: controlclient.ConnectModelRequest{
						WriteBase: controlclient.WriteBase{OperationID: "model-connect-operation-1", ExpectedRevision: &value},
						Config:    controlclient.ConnectConfig{Provider: "ollama", Model: "mimo"},
					},
					decoded: &controlclient.ConnectModelRequest{},
				},
				{
					name: "use",
					request: controlclient.UseModelRequest{
						WriteBase: controlclient.WriteBase{OperationID: "model-use-operation-1", ExpectedRevision: &value},
						Model:     "mimo",
					},
					decoded: &controlclient.UseModelRequest{},
				},
				{
					name: "delete",
					request: controlclient.DeleteModelRequest{
						WriteBase: controlclient.WriteBase{OperationID: "model-delete-operation-1", ExpectedRevision: &value},
						Model:     "mimo",
					},
					decoded: &controlclient.DeleteModelRequest{},
				},
			}
			for _, modelRequest := range modelRequests {
				...
				var base controlclient.WriteBase
			...
			sessionModelJSON := mustMarshalWire(t, controlclient.SessionModelRequest{
			...
			var decodedSessionModel controlclient.SessionModelRequest
			...
			handoffJSON := mustMarshalWire(t, controlclient.HandoffAgentRequest{
				WriteBase: controlclient.WriteBase{
			...
			var decodedHandoff controlclient.HandoffAgentRequest
			...
			bindingJSON := mustMarshalWire(t, controlclient.BindAgentBindingRequest{
				WriteBase: controlclient.WriteBase{OperationID: "agent-binding-operation-1", ExpectedRevision: &value},
			...
			var decodedBinding controlclient.BindAgentBindingRequest
			...
			resultJSON := mustMarshalWire(t, controlclient.CommandResult{
				...
				Resource: &controlclient.CommandResource{
					Kind: controlclient.CommandResourceACPPreparation, Ref: "preparation-1", Digest: "digest-1",
				},
			})
	...
	requests := []struct {
	...
	}{
		{
			...
			request: controlclient.PrepareACPRequest{
				WriteBase: controlclient.WriteBase{OperationID: "prepare-1", ExpectedRevision: &revision},
				Request:   controlagents.ACPPrepareRequest{AdapterID: "codex", Launcher: controlagents.LauncherChoiceNPX},
			},
			decoded: &controlclient.PrepareACPRequest{},
		},
		{
			name:   "prepare-auth",
			schema: "PrepareACPAuthenticationRequest",
			request: controlclient.PrepareACPAuthenticationRequest{
				WriteBase:         controlclient.WriteBase{OperationID: "prepare-auth-1", ExpectedRevision: &revision},
				PreparationRef:    "preparation-1",
				PreparationDigest: digest,
				MethodID:          "login",
			},
			decoded: &controlclient.PrepareACPAuthenticationRequest{},
		},
		{
			name:   "connect",
			schema: "ConnectACPRequest",
			request: controlclient.ConnectACPRequest{
				WriteBase:         controlclient.WriteBase{OperationID: "connect-1", ExpectedRevision: &revision},
				PreparationRef:    "preparation-1",
				PreparationDigest: digest,
				ConfigValues:      map[string]string{"effort": "high"},
			},
			decoded: &controlclient.ConnectACPRequest{},
		},
	}
...
func TestParticipantAndCompactWriteRequestsPreserveExpectedRevisionAsDecimal(t *testing.T) {
	...
	requests := []struct {
	...
	}{
		{
			...
			request: controlclient.CompactSessionRequest{WriteBase: write},
			decoded: &controlclient.CompactSessionRequest{},
		},
		{
			name:   "start-participant",
			schema: "StartParticipantRequest",
			request: controlclient.StartParticipantRequest{
...
func TestDisconnectWirePreservesHostRevisionAtUint64Boundary(t *testing.T) {
	...
	request := controlclient.DisconnectACPRequest{
		WriteBase: controlclient.WriteBase{OperationID: "disconnect-1", ExpectedRevision: &revision},
		AgentID:   "codex",
	}
	...
	var decodedRequest controlclient.DisconnectACPRequest
	...
	var decodedSnapshot controlclient.DisconnectCandidatesSnapshot
