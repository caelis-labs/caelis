...
func TestClientReconnectAllowsOwnerExactTargetObservationOfSystemManagedSession(t *testing.T) {
	t.Parallel()
	...
	// Owner exact-target Reconnect is the Host-side observation path used by
	// product ACP without granting RoleSystemSessionRuntime to Surface tokens.
	got, err := client.Reconnect(context.Background(), Principal{ID: "owner"}, ReconnectRequest{SessionID: "system-child"})
	if err != nil {
		t.Fatal(err)
	}
	if got.State.SessionID != "system-child" || got.Subscription != subscription {
		t.Fatalf("Reconnect() = %#v, want owner exact-target system-managed feed", got)
	}
	if got := subscription.closeCalls.Load(); got != 0 {
		t.Fatalf("subscription Close() calls = %d, want 0", got)
	}
