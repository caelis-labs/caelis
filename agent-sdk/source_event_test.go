...
func TestCloneSourceEventClonesCanonicalOnly(t *testing.T) {
	...
	original := SourceEvent{
		...
		Native:                           native,
		CanonicalContentAlreadyPublished: PublishedAssistantMessage,
	}
	...
	if !cloned.CanonicalContentAlreadyPublished.Has(PublishedAssistantMessage) {
		t.Fatal("cloned source event lost live content ownership")
	}
