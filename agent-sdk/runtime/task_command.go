...
func commandRequestDigest(req taskapi.CommandStartRequest) (string, error) {
	...
	}{strings.TrimSpace(req.Command), strings.TrimSpace(req.Workdir), req.TTY, req.Timeout, strings.TrimSpace(req.ParentCall), constraints}
...
func (tm *taskRuntime) StartCommand(
	...
	createdTask := &commandTask{
		...
		metadata: map[string]any{
			...
			"parent_tool":                 shell.RunCommandToolName,
...
func (tm *taskRuntime) startClaimedCommand(
	...
	if statusErr != nil {
		status = sandbox.SessionStatus{
			SessionRef: sessionHandle.Ref(), Terminal: sessionHandle.Terminal(),
			Running: true, UpdatedAt: now,
		}
	}
	task.supportsInput = status.SupportsInput
	...
	snapshot, err := tm.waitCommandCompletion(ctx, task, req.Yield)
	return snapshot, err
}
...
func (tm *taskRuntime) snapshotExistingCommand(ctx context.Context, task *commandTask, yield time.Duration) (taskapi.Snapshot, error) {
	...
	return tm.waitCommandCompletion(ctx, task, yield)
}
