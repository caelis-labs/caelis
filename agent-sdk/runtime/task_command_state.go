...
func (tm *taskRuntime) rehydrateCommandTask(entry *taskapi.Entry) (*commandTask, error) {
	...
	task := &commandTask{
		...
		tty:           taskSpecBool(entry.Spec, "tty"),
...
func (t *commandTask) entrySnapshot(now time.Time) *taskapi.Entry {
	...
	return &taskapi.Entry{
		...
		State:          t.state,
		Running:        t.running,
		SupportsInput:  t.supportsInput && t.running,
		SupportsCancel: true,
		CreatedAt:      t.createdAt,
		UpdatedAt:      now,
		Lease:          taskapi.CloneLease(t.lease),
...
func (t *commandTask) snapshotWithoutSession(now time.Time) taskapi.Snapshot {
	...
	t.mu.Lock()
	defer t.mu.Unlock()
	return t.snapshotLocked(sandbox.SessionStatus{Running: t.running, SupportsInput: t.supportsInput && t.running, UpdatedAt: now})
}
