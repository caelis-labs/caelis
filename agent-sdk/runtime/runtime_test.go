...
func TestNewRequiresInjectedACPControlPlaneWhenAssemblyAgentsConfigured(t *testing.T) {
	t.Parallel()

	sessions, _ := newTestSessionService(t, "sess-missing-control-plane")
	agents := assembly.ResolvedAssembly{
	...
	if err == nil {
		t.Fatal("expected missing injected ACP control plane to fail")
	}
	if !strings.Contains(err.Error(), "Controllers is required") {
		t.Fatalf("New() error = %v, want missing Controllers rejection", err)
	}

	_, err = New(testConfigWithACPForwarder(Config{
		Sessions: sessions,
		AgentFactory: chat.Factory{
			SystemPrompt: "Be terse.",
		},
	...
	if err == nil {
		t.Fatal("expected missing agent config updater to fail")
	}
	if !strings.Contains(err.Error(), "agent config updater is required") {
		t.Fatalf("New() error = %v, want missing updater rejection", err)
	}
}
