...
func TestRuntimePolicyWriteOutsideAllowedRootsRequiresApproval(t *testing.T) {
	...
	requester := approvalRequesterFunc(func(_ context.Context, req agent.ApprovalRequest) (agent.ApprovalResponse, error) {
		sawApproval = true
		if req.ModelStep == nil || strings.TrimSpace(req.ModelStep.ID) == "" || req.ModelStep.Index != 0 || req.ModelStep.CallCount != 1 {
			t.Fatalf("approval model step = %#v, want one call in a non-empty model step", req.ModelStep)
		}
...
func TestRuntimeRunCommandClosesStdinAndTaskWriteRejectsInput(t *testing.T) {
	t.Parallel()
	...
	runCommandResult := callRuntimeRunCommandTool(t, runCommandTool, map[string]any{
		"command":       shellStdinEOFThenSleepForTest(10 * time.Second),
		"workdir":       ".",
	...
	_, err = taskTool.Call(context.Background(), tool.Call{ID: "task-write", Name: tasktool.ToolName, Input: raw})
	if err == nil || !strings.Contains(err.Error(), "does not accept stdin") {
		t.Fatalf("Task write error = %v, want stdin rejection", err)
	}
...
func TestRuntimeTaskReadIsImmediateAndWaitObservesNewOutput(t *testing.T) {
	...
	if elapsed := time.Since(start); elapsed >= 500*time.Millisecond {
		t.Fatalf("TASK read elapsed = %v, want an immediate snapshot", elapsed)
	}
	...
	if latest, _ := payload["latest_output"].(string); strings.Contains(latest, "wait-ready") {
		t.Fatalf("task read latest_output = %q, want no future output", latest)
	}
...
func TestTaskToolPayloadReturnsCompletedCommandTerminalStreams(t *testing.T) {
	payload := taskToolPayload(taskapi.Snapshot{
		...
		State:    taskapi.StateCompleted,
		Running:  false,
		Metadata: map[string]any{"parent_call": "command-call-1"},
...
func TestTaskSnapshotToolResultKeepsRunningTerminalCursorInMetaOnly(t *testing.T) {
	...
	if got := payload["supports_input"]; got != true {
		t.Fatalf("payload[supports_input] = %#v, want true while running", got)
	}
}
...
func TestTaskSnapshotToolResultKeepsSubagentTerminalRefInMetaOnly(t *testing.T) {
	...
	if got := payload["activity_cursor"]; got != float64(17) {
		t.Fatalf("payload[activity_cursor] = %#v, want 17", got)
	}
...
func TestRuntimeTaskWaitReturnsEveryUnreadTurnFinalOnce(t *testing.T) {
	...
	_, activeSession, runtime := newRuntimeRunCommandToolTestHarness(t)
	child := &subagentTask{
		ref:        taskapi.Ref{TaskID: "task-multi-final", SessionID: "child-session"},
		sessionRef: activeSession.SessionRef,
		handle:     "ella",
		turnSeq:    1,
		state:      taskapi.StateRunning,
		running:    true,
		result:     map[string]any{"handle": "ella", "state": string(taskapi.StateRunning)},
		metadata:   map[string]any{"handle": "ella", "state": string(taskapi.StateRunning)},
	}
	child.mu.Lock()
	...
	child.mu.Unlock()
	runtime.tasks.mu.Lock()
	runtime.tasks.subagents[child.ref.TaskID] = child
	runtime.tasks.mu.Unlock()
	...
	if payload["state"] != "running" || payload["output_preview"] != "third Turn in progress" ||
		...
		t.Fatalf("first Task wait payload = %#v, want current preview plus newest compatibility alias", payload)
	}

	second := callRuntimeTaskTool(t, taskTool, map[string]any{"action": "read", "handle": "ella"})
	payload = testToolResultPayload(t, second)
	if _, exists := payload["final_responses"]; exists {
		t.Fatalf("second Task read repeated final_responses: %#v", payload)
	}
	if _, exists := payload["final_message"]; exists {
		t.Fatalf("second Task read repeated final_message: %#v", payload)
	}
	if payload["state"] != "running" || payload["output_preview"] != "third Turn in progress" {
