...
func (s steeringACPController) SteerParticipant(ctx context.Context, req controller.ParticipantSteerRequest) error {
	if s.steerParticipant == nil {
		return controller.ErrParticipantSteeringUnsupported
	}
	return s.steerParticipant(ctx, req)
}
...
func testConfigWithACPForwarder(cfg Config) Config {
	if cfg.ControllerEventForwarder == nil && requiresControllerForwarder(cfg) {
		cfg.ControllerEventForwarder = testControllerForwarder{sessions: cfg.Sessions}
	}
	if cfg.ControllerContextRouter == nil && (cfg.Controllers != nil || cfg.Subagents != nil) {
		cfg.ControllerContextRouter = testContextRouter{sessions: cfg.Sessions}
	}
	if cfg.ControllerRecovery == nil && cfg.Controllers != nil {
		cfg.ControllerRecovery = testRecoveryCoordinator{}
	}
	return cfg
}

type testRecoveryCoordinator struct{}

func (testRecoveryCoordinator) ReattachController(_ context.Context, req controller.RecoveryRequest) (session.Session, error) {
	return session.CloneSession(req.Session), nil
}

type testContextRouter struct {
...
func (r testContextRouter) contextTransfer(ctx context.Context, ref session.SessionRef, sinceSeq uint64, excludeTurnID string) (agent.ContextTransfer, uint64, error) {
	...
	for i, event := range events {
		...
		if excludeTurnID != "" && event.Scope != nil && strings.TrimSpace(event.Scope.TurnID) == strings.TrimSpace(excludeTurnID) {
			continue
		}
		typeOf := session.EventTypeOf(event)
...
func testIsControllerNarrativeChunk(event *session.Event) bool {
	...
	switch session.ProtocolSessionUpdateType(event) {
...
func testShouldPersistExternalControllerEvent(event *session.Event) bool {
...
type contextProbeModel struct {
	...
	checkCompactionStream       bool
	wantCompactionStream        bool
...
func shellStdinEOFThenSleepForTest(delay time.Duration) string {
	if goruntime.GOOS == "windows" {
		ms := max(int(delay.Milliseconds()), 1)
		return fmt.Sprintf("$text = [Console]::In.ReadToEnd(); if ($text.Length -eq 0) { [Console]::Out.WriteLine('stdin-eof') } else { [Console]::Out.WriteLine('unexpected-input') }; Start-Sleep -Milliseconds %d", ms)
	}
	return fmt.Sprintf("if IFS= read -r line; then printf 'unexpected-input\\n'; else printf 'stdin-eof\\n'; fi; sleep %.3f", float64(delay)/float64(time.Second))
}
...
func (s *yieldProbeSandboxSession) AwaitOutput(ctx context.Context, cursor sandbox.OutputCursor) (sandbox.OutputObservation, error) {
	...
	s.awaitOnce.Do(func() {
		...
		s.stdout = s.awaitOutput
		next.Stdout = cursor.Stdout + int64(len([]byte(s.awaitOutput)))
		if s.onOutput != nil {
			s.onOutput(sandbox.OutputChunk{Stream: "stdout", Text: s.awaitOutput, Cursor: next.Stdout})
		}
		emitted = true
	})
	if s.awaitErr != nil {
		return sandbox.OutputObservation{}, s.awaitErr
	}
	if emitted {
		return sandbox.OutputObservation{Cursor: next, Status: status}, nil
	}
