...
func TestAutoCompactDecisionBeforeModelRequestStillUsesProviderHighWater(t *testing.T) {
	...
	if decision.Usage.Source != compact.UsageSourceProvider {
		t.Fatalf("usage source = %q, want provider", decision.Usage.Source)
	}
	if decision.Usage.TotalTokens < 244000 {
		t.Fatalf("usage total = %d, want provider high-water baseline", decision.Usage.TotalTokens)
	}
}

func TestAutoCompactDecisionBeforeModelRequestFallsBackForDifferentProviderIdentity(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name                  string
		llm                   model.LLM
		clearSnapshotIdentity bool
	}{
		{
			name: "provider mismatch",
			llm: identifiedCompactionModel{
		...
		},
		{
			...
			llm:  staticModel{text: "ok"},
		},
		{
			name:                  "unknown stored identity",
			llm:                   matchingProviderGateModel(),
			clearSnapshotIdentity: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.clearSnapshotIdentity {
				providerEvent.Invocation = nil
				sdkMeta := nestedMap(providerEvent.Meta, "caelis", "sdk")
				delete(sdkMeta, "provider")
				delete(sdkMeta, "model")
			}
			appendTestEvent(t, runtime.sessions, activeSession.SessionRef, providerEvent)
			req := &model.Request{Messages: []model.Message{
				model.NewTextMessage(model.RoleUser, strings.Repeat("x", 1_000_000)),
			}}
			...
			if decision.Reason != "model_request_context_limit" {
				t.Fatalf("decision = %+v, want full-request local gate after identity mismatch", decision)
			}
			if decision.Usage.Source != compact.UsageSourceEstimated {
				t.Fatalf("usage source = %q, want estimated fallback", decision.Usage.Source)
			}
			if decision.Usage.TotalTokens < decision.RequestTokens {
				t.Fatalf("usage/request tokens = %d/%d, want full request estimate retained", decision.Usage.TotalTokens, decision.RequestTokens)
			}
		})
	}
}
