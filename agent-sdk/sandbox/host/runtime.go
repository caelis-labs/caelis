...
func (r *Runtime) Run(ctx context.Context, req sandbox.CommandRequest) (sandbox.CommandResult, error) {
	...
	if req.TTY {
		...
		if err != nil {
			return sandbox.CommandResult{Route: sandbox.RouteHost, Backend: sandbox.BackendHost}, err
		}
...
func (r *Runtime) Start(ctx context.Context, req sandbox.CommandRequest) (sandbox.Session, error) {
	...
	if req.TTY {
		terminal, ttyProcess, err := startHostTTYCommand(cmd)
		if err != nil {
			cancel()
			return nil, fmt.Errorf("impl/sandbox/host: start tty: %w", err)
		}
		session.stdin = terminal
		session.ttyProcess = ttyProcess
		r.mu.Lock()
		r.sessions[sessionID] = session
		r.mu.Unlock()
		session.wg.Add(1)
		go session.readStream(terminal, "stdout")
		go session.waitForExit()
		if ttyProcess != nil {
			go func() {
				select {
				case <-cmdCtx.Done():
					_ = ttyProcess.Terminate()
				case <-session.done:
				}
			}()
		}
...
func (s *hostSession) Terminate(ctx context.Context) error {
	...
	s.mu.Lock()
	cmd := s.cmd
	ttyProcess := s.ttyProcess
	running := s.running
	if !running || (ttyProcess == nil && (cmd == nil || cmd.Process == nil)) {
		s.mu.Unlock()
		return nil
	}
	s.terminating = true
	...
	if ttyProcess != nil {
		terminateErr = ttyProcess.Terminate()
	} else {
...
func (s *hostSession) waitForExit() {
	...
	var err error
	if s.ttyProcess != nil {
		exitCode, err = s.ttyProcess.Wait()
		s.ttyProcess.CloseAfterExit()
	} else {
		err = s.cmd.Wait()
...
func (s *hostSession) cleanupProcessGroupAfterExit() {
	if s == nil || s.ttyProcess != nil || s.cmd == nil || s.cmd.Process == nil {
		return
	}
