...
func TestRuntimeSessionClosesStdinForNonTTYCommand(t *testing.T) {
	t.Parallel()
	...
	session, err := rt.Start(context.Background(), commandRequest("if read line; then printf 'got:%s' \"$line\"; else printf 'stdin-eof'; fi; sleep 0.2"))
	if err != nil {
		t.Fatalf("Start() error = %v", err)
	}
	...
	if status.SupportsInput {
		t.Fatal("status.SupportsInput = true, want false")
	}
	if err := session.WriteInput(context.Background(), []byte("demo\n")); err == nil {
		t.Fatal("WriteInput() error = nil, want stdin rejection")
	}
	...
	if got := result.Stdout; !strings.Contains(got, "stdin-eof") {
		t.Fatalf("stdout = %q, want stdin-eof", got)
	}
}

func TestRuntimeSupportsInteractiveTTY(t *testing.T) {
	t.Parallel()
	...
	if !rt.Describe().Capabilities.TTY {
		t.Fatal("Describe().Capabilities.TTY = false, want true")
	}
	req := commandRequest("test -t 0 && IFS= read -r line && printf 'got:%s' \"$line\"")
	req.TTY = true
	...
	if status.Running || status.SupportsInput {
		t.Fatalf("terminal Status() = %+v, want no running input capability", status)
	}
	result, err := session.Result(context.Background())
	if err != nil {
		t.Fatalf("Result() error = %v", err)
	...
	if result.Stderr != "" {
		t.Fatalf("stderr = %q, want tty output merged into stdout", result.Stderr)
	}
}

func TestRuntimeRunSupportsTTY(t *testing.T) {
	t.Parallel()
	...
	if err != nil {
		t.Fatalf("Run(TTY=true) error = %v; result=%+v", err, result)
	}
	if !strings.Contains(result.Stdout, "got:x") || result.Stderr != "" {
		t.Fatalf("Run(TTY=true) result = %+v, want merged got:x output", result)
	}
}
