...
// AsyncSession represents an asynchronous shell session that streams output
// independently of the calling goroutine. Only TTY sessions keep stdin open;
// non-TTY commands start with stdin closed and reject WriteInput.
...
func (s *AsyncSession) Start() error {
	...
	if s.tty {
		terminal, err := startTTYCommand(cmd)
		if err != nil {
			return fmt.Errorf("failed to start command with tty: %w", err)
		}
		s.cmd = cmd
		s.stdinWriter = terminal
		s.readersWg.Add(1)
		go s.readOutput(terminal, "stdout", s.stdoutBuffer)
		go s.waitForExit()
	...
	// Non-TTY commands deliberately inherit no stdin. os/exec connects a nil
	// Stdin to the null device, so reads observe EOF instead of leaving a
	// background command waiting for input that the caller cannot provide.
...
func (s *AsyncSession) closeStdin() {
...
func (s *AsyncSession) GetResult() (sandbox.CommandResult, error) {
	if !s.exited.Load() {
		return sandbox.CommandResult{}, errors.New("session has not exited yet")
	}
