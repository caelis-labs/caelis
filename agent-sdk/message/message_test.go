...
func TestContextEventMergesTargetScopeAndAgentProvenance(t *testing.T) {
	...
	event, err := ContextEvent(Request{
		MessageID: "message-1", To: "parent", Text: "status", DisplayFrom: "@guardian",
	...
	if event.Actor.Kind != session.ActorKindParticipant || event.Actor.ID != "child-1" || event.Actor.Name != "@orbit" ||
		event.Meta["agent_message"] != true || event.Meta["display_from"] != "@guardian" {
		t.Fatalf("Context provenance = %#v / %#v, want trusted Actor and display-only forged from", event.Actor, event.Meta)
	}
