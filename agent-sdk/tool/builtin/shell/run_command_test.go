...
func TestRunCommandDefinitionExposesMinimalArguments(t *testing.T) {
	...
	ttyProperty, _ := properties["tty"].(map[string]any)
	if got := ttyProperty["default"]; got != false {
		t.Fatalf("tty default = %#v, want false", got)
	}
...
func TestRunCommandCallUsesConfiguredHardTimeoutOnly(t *testing.T) {
	...
	if !last.TTY {
		t.Fatal("command TTY = false, want explicit tty=true propagation")
	}
}
