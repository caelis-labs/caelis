		...
		InputSchema: map[string]any{
			...
			"properties": map[string]any{
				"action": map[string]any{
					...
					"description": "read inspects now; wait observes for up to one minute and waits on commands until terminal state or the window expires; write sends stdin only to an input-capable command; cancel interrupts the current activity.",
				},
