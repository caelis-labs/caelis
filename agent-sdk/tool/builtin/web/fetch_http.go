	...
	tr.DialContext = func(ctx context.Context, network string, address string) (net.Conn, error) {
		...
		for _, addr := range ips {
			if isBlockedFetchIP(addr.IP) {
				return nil, fmt.Errorf("refusing to fetch private or local network address %s", addr.IP.String())
			}
		}
		var lastErr error
		for _, addr := range ips {
...
func (t *FetchTool) fetch(ctx context.Context, u *url.URL, format string) (fetchResponseMeta, []byte, error) {
	...
	if resp.StatusCode >= 400 {
		return fetchResponseMeta{}, nil, fmt.Errorf("HTTP status %d", resp.StatusCode)
	}
	if resp.ContentLength > t.cfg.MaxResponseBytes {
		return fetchResponseMeta{}, nil, fmt.Errorf("response too large (%d bytes, limit %d)", resp.ContentLength, t.cfg.MaxResponseBytes)
	}
	...
	if int64(len(raw)) > t.cfg.MaxResponseBytes {
		return fetchResponseMeta{}, nil, fmt.Errorf("response too large (limit %d bytes)", t.cfg.MaxResponseBytes)
	}
...
func rejectPrivateFetchTarget(ctx context.Context, u *url.URL) error {
	...
	for _, addr := range ips {
		if isBlockedFetchIP(addr.IP) {
			return fmt.Errorf("refusing to fetch private or local network address %s", addr.IP.String())
		}
	}
	return nil
}
...
func normalizeFetchFormat(format string) (string, error) {
	...
	default:
		return "", fmt.Errorf("format must be one of markdown, text, or html")
	}
}
