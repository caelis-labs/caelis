...
func TestGeneratedSessionTitleBoundsLargeUnicodeInput(t *testing.T) {
	t.Parallel()

	message := model.NewTextMessage(model.RoleUser, "model input")
	large := strings.Repeat("界", 1<<20)
	got := GeneratedSessionTitle(&Event{
		Type: EventTypeUser, Visibility: VisibilityCanonical, Message: &message, Text: large,
	})
	want := strings.Repeat("界", 80)
	if got != want {
		t.Fatalf("GeneratedSessionTitle(large) runes = %d, want 80", utf8.RuneCountInString(got))
	}

	invalid := strings.Repeat("a", 79) + "\xfftail"
	got = GeneratedSessionTitle(&Event{
		Type: EventTypeUser, Visibility: VisibilityCanonical, Message: &message, Text: invalid,
	})
	want = strings.Repeat("a", 79) + "\uFFFD"
	if got != want || !utf8.ValidString(got) {
		t.Fatalf("GeneratedSessionTitle(invalid) = %q, valid=%v, want %q", got, utf8.ValidString(got), want)
	}
}
