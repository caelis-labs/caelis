...
func UsageSnapshotFromSessionEvent(event *Event) *UsageSnapshot {
	...
	for _, meta := range []map[string]any{semanticUsageMetadata(event), event.Meta} {
		...
		if ok {
			...
			return usageSnapshotWithContext(event, meta, usageSnapshotFromPayload(payload, firstNonEmptyString(usageProviderFromMetadata(payload), provider)))
		}
		if raw := nestedAny(meta, "caelis", "sdk", "usage"); raw != nil {
			if payload, ok := raw.(map[string]any); ok {
				if usage := usageSnapshotFromPayload(payload, firstNonEmptyString(usageProviderFromMetadata(payload), provider)); usage != nil {
					return usageSnapshotWithContext(event, meta, usage)
				}
			}
		}
...
func usageSnapshotFromPayload(payload map[string]any, provider string) *UsageSnapshot {
	...
	contextWindowTokens := firstNonZeroInt(
		intValue(payload["context_window_tokens"]),
		intValue(payload["context_window"]),
		intValue(payload["contextWindowTokens"]),
		intValue(payload["contextWindow"]),
		intValue(nestedAny(payload, "usage_metadata", "context_window_tokens")),
		intValue(nestedAny(payload, "usageMetadata", "contextWindowTokens")),
	)
	...
	cacheSeparated := model.ProviderSeparatesCachedInputForUsage(provider, payload)
	...
	if usage.PromptTokens == 0 && usage.CachedInputTokens == 0 && usage.CompletionTokens == 0 && usage.ReasoningTokens == 0 && usage.TotalTokens == 0 && usage.ContextWindowTokens == 0 {
		return nil
	}
	return usage
}
...
func providerSeparatesCachedInputForSnapshot(provider string, usage UsageSnapshot) bool {
	if model.ProviderSeparatesCachedInput(provider) {
		return true
	}
...
func semanticUsageMetadata(event *Event) map[string]any {
	if event == nil {
		return nil
	}
	return event.Meta
...
func usageSnapshotWithContext(event *Event, meta map[string]any, usage *UsageSnapshot) *UsageSnapshot {
	if usage == nil {
		return nil
	}
	if usage.ContextWindowTokens <= 0 {
		usage.ContextWindowTokens = firstNonZeroInt(
			contextWindowFromSessionEvent(event),
			contextWindowFromMetadata(meta),
		)
	}
	return usage
}

func contextWindowFromSessionEvent(event *Event) int {
	if event == nil || event.Invocation == nil {
		return 0
	}
	return event.Invocation.ContextWindowTokens
}

func contextWindowFromMetadata(meta map[string]any) int {
	if len(meta) == 0 {
		return 0
	}
	return firstNonZeroInt(
		intValue(meta["context_window_tokens"]),
		intValue(meta["context_window"]),
		intValue(nestedAny(meta, "caelis", "sdk", "context_window_tokens")),
		intValue(nestedAny(meta, "caelis", "sdk", "context_window")),
		intValue(nestedAny(meta, "caelis", "invocation", "context_window_tokens")),
		intValue(nestedAny(meta, "caelis", "invocation", "context_window")),
	)
}
