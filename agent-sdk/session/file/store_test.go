...
func TestStoreConditionalAppendConformance(t *testing.T) {
	t.Parallel()
	sessiontest.ConditionalAppendConformance(t, func(t *testing.T) sessiontest.ConditionalAppendStore {
		return NewStore(Config{RootDir: t.TempDir()})
	})
}
...
func TestStoreGeneratedTitleTruncatesAtUnicodeBoundary(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	store := NewStore(Config{
		RootDir:            root,
		SessionIDGenerator: func() string { return "sess-unicode-title" },
	})
	ctx := context.Background()
	...
	if len(listed.Sessions) != 1 || listed.Sessions[0].Title != want || loaded.Session.Title != want {
		t.Fatalf("round-trip titles = list:%#v load:%q, want %q", listed.Sessions, loaded.Session.Title, want)
	}
	if !utf8.ValidString(listed.Sessions[0].Title) || utf8.RuneCountInString(listed.Sessions[0].Title) != 80 {
		t.Fatalf("listed title = %q, valid=%v runes=%d, want valid 80-rune title", listed.Sessions[0].Title, utf8.ValidString(listed.Sessions[0].Title), utf8.RuneCountInString(listed.Sessions[0].Title))
	}
}
