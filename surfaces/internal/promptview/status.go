...
func FormatDoctorSnapshot(status controlstatus.StatusSnapshot) string {
	...
	setup := controlstatus.SandboxSetupViewFromStatus(status)
	blocker := strings.TrimSpace(firstNonEmpty(status.SandboxStatus.FallbackReason, setup.SetupError, setup.GlobalDetail, setup.WorkspaceDetail))
	switch {
	case blocker != "" && (setup.GlobalRequired || setup.WorkspaceRequired || status.SandboxStatus.FallbackReason != "" || setup.SetupError != ""):
		lines = append(lines, "  blocked sandbox: "+compactStatusDetail(blocker, 180))
		if hint := strings.TrimSpace(status.SandboxStatus.InstallHint); hint != "" {
			lines = append(lines, "  fix: "+compactStatusDetail(hint, 180))
		}
	case status.SandboxStatus.FullAccessMode:
		lines = append(lines, "  warn sandbox: host escape (YOLO)")
	case status.SandboxStatus.HostExecution:
		lines = append(lines, "  warn sandbox: "+firstNonEmpty(status.SandboxStatus.SecuritySummary, sandbox, "host execution"))
	case sandbox != "":
...
// A configured window alone is not usage: hide the label until a model
// response reports a positive token count.
