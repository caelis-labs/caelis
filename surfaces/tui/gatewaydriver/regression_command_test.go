...
func newRegressionDriver(t *testing.T) (*GatewayDriver, *gatewayapp.Stack) {
	t.Helper()
	...
	driver, err := newGatewayDriverFromGatewayAppStack(ctx, stack, "regression-session", "surface", "ollama/llama3")
	if err != nil {
...
func TestRegressionCommandNewDriverWithSandboxConfig(t *testing.T) {
	...
	driver, err := newGatewayDriverFromGatewayAppStack(ctx, stack, "sandbox-session", "surface", "ollama/llama3")
	if err != nil {
		t.Fatalf("newGatewayDriverFromGatewayAppStack() error = %v", err)
	}
