...
func newCommandExecDriver(t *testing.T, modelCfg gatewayapp.ModelConfig) (*GatewayDriver, *gatewayapp.Stack) {
	t.Helper()
	...
	driver, err := newGatewayDriverFromGatewayAppStack(ctx, stack, "cmd-exec-session", "surface", modelCfg.Provider+"/"+modelCfg.Model)
	if err != nil {
...
func TestRegressionCommandExecModelUseWithReasoning(t *testing.T) {
	...
	driver, err := newGatewayDriverFromGatewayAppStack(ctx, stack, "reasoning-session", "surface", "ollama/llama3")
	if err != nil {
		t.Fatalf("newGatewayDriverFromGatewayAppStack() error = %v", err)
	}
...
func TestRegressionCommandExecDeleteCurrentModelClearsStatus(t *testing.T) {
	...
	driver, err := newGatewayDriverFromGatewayAppStack(ctx, stack, "delete-current-session", "surface", "")
	if err != nil {
		t.Fatalf("newGatewayDriverFromGatewayAppStack() error = %v", err)
	}
...
func TestRegressionCommandExecModelReasoningCompletion(t *testing.T) {
	...
	driver, err := newGatewayDriverFromGatewayAppStack(ctx, stack, "reasoning-completion-session", "surface", "ollama/llama3")
	if err != nil {
		t.Fatalf("newGatewayDriverFromGatewayAppStack() error = %v", err)
	}
