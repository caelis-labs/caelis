...
func TestSplitDiffCountTokens(t *testing.T) {
	path, added, removed, ok := SplitDiffCountTokens("process_alive.go +21 -0")
	if !ok || path != "process_alive.go" || added != "+21" || removed != "-0" {
		t.Fatalf("SplitDiffCountTokens() = %q %q %q %v", path, added, removed, ok)
	}
	if _, _, _, ok := SplitDiffCountTokens("git status --short"); ok {
		t.Fatal("expected no diff-count suffix")
	}
}
