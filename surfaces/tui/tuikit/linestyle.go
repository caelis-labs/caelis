...
func renderToolCallSuffix(toolName string, suffix string, theme Theme) string {
	if !isEditToolDisplayName(toolName) {
		return theme.ToolArgsStyle().Render(LinkifyText(suffix, theme.LinkStyle()))
	}
	path, added, removed, ok := SplitDiffCountTokens(suffix)
	if !ok || path == "" {
		return theme.ToolArgsStyle().Render(LinkifyText(suffix, theme.LinkStyle()))
	}
	return theme.ToolArgsStyle().Render(LinkifyText(path, theme.LinkStyle())) + " " + StyleDiffCountTokens(added, removed, theme)
}
...
func renderToolResultSuffix(toolName string, suffix string, theme Theme) string {
	...
	return StyleDiffCountTokens(added, removed, theme)
}
...
// SplitDiffCountTokens extracts a trailing "+N -M" suffix used by edit headers.
func SplitDiffCountTokens(text string) (prefix string, added string, removed string, ok bool) {
	...
	if !isSignedCountToken(added, '+') || !isSignedCountToken(removed, '-') {
		return "", "", "", false
	}
	prefix = strings.TrimSpace(strings.Join(fields[:len(fields)-2], " "))
	return prefix, added, removed, true
}

func isSignedCountToken(token string, sign byte) bool {
...
// StyleDiffCountTokens colors "+N" / "-M" with the same add/remove hues as diffs.
func StyleDiffCountTokens(added string, removed string, theme Theme) string {
	return theme.DiffAddStyle().Render(added) + " " + theme.DiffRemoveStyle().Render(removed)
}
