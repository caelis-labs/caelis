...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case mentionCompletionResultMsg:
		return m.handleMentionCompletionResultMsg(typed)
	...
	case spinner.TickMsg:
		...
		if m.animationIndicatorActive() && !m.noAnimation {
			prevDim := subagentOutputPulseDim(m.spinner.View())
			var cmd tea.Cmd
			m.spinner, cmd = m.spinner.Update(msg)
			if cmd != nil {
				m.spinnerTickScheduled = true
			}
			if prevDim != subagentOutputPulseDim(m.spinner.View()) && m.turnRunning() {
				m.syncViewportContent()
			}
			return m, cmd
		}
