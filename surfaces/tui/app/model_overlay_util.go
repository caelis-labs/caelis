...
func (m *Model) normalizeFullscreenFrameWithTopTrim(view string) (string, int) {
	...
	m.diag.FullscreenNormalizeCalls++
	return normalizeFullscreenFrameWithTopTrim(view, m.width, m.height)
}
...
func normalizeFullscreenFrameLine(line string, width int) string {
	...
	guard := lineNeedsRepaintGuard(line)
	...
	return protectRepaintGuardLine(line, width)
}
...
// protectRepaintGuardLine pins a non-EL-clearable cell at the end of the line.
// Ultraviolet uses ICH/DCH when the last cell is a plain space. Ghostty erases a
// CJK glyph when ICH splits it, which shows up as missing characters and shifted
// alignment until a full redraw. The conceal-space sentinel makes that last cell
// ineligible for EL/ICH so the renderer rewrites the changed range in place.
func protectRepaintGuardLine(line string, width int) string {
	if line == "" || width <= 1 {
		return line
	}
...
func lineNeedsRepaintGuard(line string) bool {
	return strings.Contains(line, wideCellRendererSentinel())
}
