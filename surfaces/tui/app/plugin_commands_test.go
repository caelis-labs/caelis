...
func TestPluginManagerSelectionUpdatesEnabledPlugins(t *testing.T) {
	...
	awaitPluginManagerSelection(context.Background(), svc, func(msg tea.Msg) {
		if notice, ok := msg.(SlashNoticeMsg); ok {
			notices = append(notices, notice.Text)
		}
	}, []controlprompt.PluginSnapshot{
	...
	if combined := strings.Join(notices, "\n"); !strings.Contains(combined, "Updated plugins") || !strings.Contains(combined, "Restart new tasks") {
		t.Fatalf("plugin selection notice = %q, want short update plus restart hint", combined)
	}
}
...
func TestSlashPluginInstallSuccess(t *testing.T) {
	...
	if !strings.Contains(combined, "Restart new tasks") {
		t.Fatalf("install notice = %q, want restart guidance", combined)
	}
}
...
func TestSlashPluginMarketplaceAddSuccess(t *testing.T) {
	...
	combined := strings.Join(notices, "\n")
	if !strings.Contains(combined, "Added marketplace demo-market") {
...
func TestSlashPluginRmSuccess(t *testing.T) {
	...
	if !strings.Contains(combined, "Restart new tasks") {
		t.Fatalf("remove notice = %q, want restart guidance", combined)
	}
}
