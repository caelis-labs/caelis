...
func TestExitCommandsQuitLocallyWithoutStartingControlWork(t *testing.T) {
	for _, line := range []string{"/exit", "/quit"} {
		t.Run(line, func(t *testing.T) {
			model := NewModel(Config{
				Commands: []string{"exit", "quit"},
				ExecuteLine: func(Submission) TaskResultMsg {
					t.Fatalf("%s reached asynchronous ExecuteLine", line)
					return TaskResultMsg{}
				},
			})

			next, cmd := model.submitInteractiveLine(line, line, nil)
			model = next.(*Model)
			if cmd == nil || !model.quit {
				t.Fatalf("submitInteractiveLine(%q) = cmd:%v quit:%v, want immediate quit", line, cmd != nil, model.quit)
			}
			if model.liveTurn.Active {
				t.Fatalf("submitInteractiveLine(%q) started a live turn", line)
			}
		})
	}
}
