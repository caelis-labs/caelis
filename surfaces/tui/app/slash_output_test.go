...
func TestSlashNoticeOutputUsesAlignedPlainText(t *testing.T) {
	...
	if !block.lines[0].Plain {
		t.Fatalf("notice lines = %#v, want aligned explicit plain text", block.lines)
	}

	single := renderSlashNoticeLines(SlashNoticeMsg{Text: "Switched to openai-codex/gpt-5.6-sol"})
	if len(single) != 1 || single[0].Text != "Switched to openai-codex/gpt-5.6-sol" || !single[0].Plain {
...
func TestExecuteControlPromptResultOmitsNewSessionSuccessNotice(t *testing.T) {
	t.Parallel()
	...
	for _, block := range model.doc.Blocks() {
		...
		if strings.Contains(slashOutputPlainForTest(slash.lines), "ready for a new session") {
			t.Fatalf("transcript still contains /new success notice: %#v", slash.lines)
		}
	}
}
