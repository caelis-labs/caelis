...
func TestHandleACPEventEnvelopeAppliesToolTerminalSequence(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCall{
			...
			Content:       []eventstream.ToolCallContent{{Type: "terminal", TerminalID: "call-1"}},
			Meta:          testMeta.WithTerminalInfo(nil, "call-1"),
		},
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind:      eventstream.KindSessionUpdate,
		...
		Update: eventstream.ToolCallUpdate{
			...
			RawInput:      map[string]any{"command": "echo ok"},
			Content:       []eventstream.ToolCallContent{{Type: "terminal", TerminalID: "call-1"}},
			Meta:          testMeta.WithTerminalOutput(nil, "call-1", "ok\n"),
		},
	})
...
func TestHandleACPEventEnvelopeDisplaysTaskOwnedRunCommandOutputOnce(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCall{
			...
			Title: "RunCommand sleep 6", Kind: eventstream.ToolKindExecute, Status: eventstream.ToolStatusInProgress,
	...
	meta := testMeta.WithRuntimeSection(acpToolNameMeta("RunCommand"), testMeta.RuntimeTask, map[string]any{
		"task_id": "task-1", "kind": "command",
		testMeta.RuntimeOutputDelta: output, "running": false, "state": "completed",
	})
	event := &session.Event{
		...
		Tool: &session.EventTool{
			ID: "command-1", Name: "RunCommand", Kind: eventstream.ToolKindExecute,
...
func TestHandleACPEventEnvelopeRendersTaskWriteInteractionAndKeepsUTF8RunCommandOutputOnce(t *testing.T) {
	...
	commandMeta := testMeta.WithRuntimeSection(acpToolNameMeta("RunCommand"), testMeta.RuntimeTask, map[string]any{
		"task_id":                      "task-4",
		testMeta.RuntimeTaskTerminalID: "terminal-4",
		"handle":                       "command-4",
		"kind":                         "command",
		"running":                      true,
		"state":                        "running",
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCall{
			SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "command-call-4",
			Title: "RunCommand read name", Kind: eventstream.ToolKindExecute, Status: eventstream.ToolStatusInProgress,
	...
	taskMeta := testMeta.WithRuntimeSection(acpToolNameMeta("Task"), testMeta.RuntimeTool, map[string]any{
		testMeta.RuntimeToolName: "Task", "action": "write",
		"input": "小明", "target_kind": "command", "target_handle": "command-4",
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCall{
			SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "task-write-4",
			Title: "Task write command-4", Kind: eventstream.ToolKindOther, Status: eventstream.ToolStatusInProgress,
			RawInput: taskInput, Meta: taskMeta,
		},
	})
	completed := eventstream.ToolStatusCompleted
	taskMeta = testMeta.WithRuntimeSection(taskMeta, testMeta.RuntimeTask, map[string]any{
		"task_id": "task-4", testMeta.RuntimeOutputDelta: output,
		"handle": "command-4", "kind": "command", "running": false, "state": "completed",
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCallUpdate{
			SessionUpdate: eventstream.UpdateToolCallInfo, ToolCallID: "task-write-4", Status: &completed,
...
func TestHandleACPEventEnvelopeRendersTaskWriteFailureWithoutInternalHandle(t *testing.T) {
	...
	taskMeta := testMeta.WithRuntimeSection(acpToolNameMeta("Task"), testMeta.RuntimeTool, map[string]any{
		testMeta.RuntimeToolName: "Task", "action": "write",
		"input": "test", "target_kind": "command", "target_handle": "command-10",
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCall{
			SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "task-write-10",
			Title: "Task write command-10", Kind: eventstream.ToolKindOther, Status: eventstream.ToolStatusInProgress,
			RawInput: taskInput, Meta: taskMeta,
		},
	})
	...
	taskMeta = testMeta.WithRuntimeSection(taskMeta, testMeta.RuntimeTask, map[string]any{
		testMeta.RuntimeOutputDelta: "existing shell output\n", "handle": "command-10", "kind": "command",
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCallUpdate{
			SessionUpdate: eventstream.UpdateToolCallInfo, ToolCallID: "task-write-10", Status: &failed,
...
func TestHandleACPEventEnvelopeRendersTerminalTaggedCommandTaskWriteAsShellInteraction(t *testing.T) {
	...
	taskMeta := testMeta.WithRuntimeSection(acpToolNameMeta("Task"), testMeta.RuntimeTool, map[string]any{
		testMeta.RuntimeToolName: "Task", "action": "write",
		"input": input, "target_kind": "command", "target_handle": "command-82",
	})
	taskMeta = testMeta.WithRuntimeSection(taskMeta, testMeta.RuntimeTask, map[string]any{
		"task_id": "task-82", testMeta.RuntimeTaskTerminalID: "terminal-82",
		"handle": "command-82", "kind": "command", "running": true, "state": "running",
	})
	taskMeta = testMeta.WithTerminalInfo(taskMeta, "terminal-82")
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCall{
			SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "task-write-82",
			Title: "Task write command-82", Kind: eventstream.ToolKindOther, Status: eventstream.ToolStatusInProgress,
			RawInput: taskInput, Content: []eventstream.ToolCallContent{{Type: "terminal", TerminalID: "terminal-82"}},
			Meta: taskMeta,
		},
	})
	completed := eventstream.ToolStatusCompleted
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCallUpdate{
			SessionUpdate: eventstream.UpdateToolCallInfo, ToolCallID: "task-write-82", Status: &completed,
...
func TestHandleACPEventEnvelopeKeepsStreamedRunCommandOutputOnEmptyFinalFrame(t *testing.T) {
	...
	finalMeta := testMeta.WithRuntimeSection(acpToolNameMeta("RunCommand"), testMeta.RuntimeTask, map[string]any{
		"task_id":                      "task-1",
		testMeta.RuntimeTaskTerminalID: "command-1",
		"output_cursor":                len([]byte(output)),
		"running":                      false,
		"state":                        "completed",
	})
	finalMeta = testMeta.WithTerminalInfo(finalMeta, "command-1")
	finalMeta = testMeta.WithTerminalExit(finalMeta, "command-1", &exitCode, nil)
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind:      eventstream.KindSessionUpdate,
		SessionID: "session-1",
		Scope:     eventstream.ScopeMain,
...
func TestTaskWaitOfFailedCommandStaysHiddenAndFoldsIntoOwner(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCall{
			...
			Title: "RunCommand go test", Kind: eventstream.ToolKindExecute, Status: eventstream.ToolStatusInProgress,
	...
	running := eventstream.ToolStatusInProgress
	commandMeta := testMeta.WithRuntimeSection(acpToolNameMeta("RunCommand"), testMeta.RuntimeTask, map[string]any{
		"task_id": "task-1", testMeta.RuntimeTaskTerminalID: "terminal-1",
		"handle": "command-2", "running": true, "state": "running",
	})
	commandMeta = testMeta.WithTerminalInfo(commandMeta, "terminal-1")
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", TurnID: "turn-1", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCallUpdate{
	...
	taskMeta := testMeta.WithRuntimeSection(acpToolNameMeta("Task"), testMeta.RuntimeTool, map[string]any{
		testMeta.RuntimeToolName: "Task", "action": "wait",
		"target_handle": "command-2", "target_kind": "command",
	})
	taskMeta = testMeta.WithRuntimeSection(taskMeta, testMeta.RuntimeTask, map[string]any{
		"task_id": "task-1", testMeta.RuntimeTaskTerminalID: "terminal-1",
		"handle": "command-2", "running": false, "state": "failed",
		testMeta.RuntimeOutputDelta: failedOutput,
	})
	taskMeta = testMeta.WithTerminalInfo(taskMeta, "terminal-1")
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", TurnID: "turn-1", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCall{
			SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "task-wait-1",
			Title: "Task wait command-2", Kind: eventstream.ToolKindOther, Status: eventstream.ToolStatusInProgress,
			RawInput: taskInput, Meta: taskMeta,
		},
	})
	failed := eventstream.ToolStatusFailed
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCallUpdate{
			SessionUpdate: eventstream.UpdateToolCallInfo, ToolCallID: "task-wait-1", Status: &failed,
			RawInput: taskInput, RawOutput: map[string]any{
...
func TestResumeUsesDurableTaskWaitResultWhenCommandTransientOutputIsMissing(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCall{
			SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "command-1",
			Title: "RunCommand long job", Kind: eventstream.ToolKindExecute, Status: eventstream.ToolStatusInProgress,
	...
	completed := eventstream.ToolStatusCompleted
	exitCode := 0
	commandFinalMeta := testMeta.WithRuntimeSection(acpToolNameMeta("RunCommand"), testMeta.RuntimeTask, map[string]any{
		"task_id": "task-1", testMeta.RuntimeTaskTerminalID: "terminal-1",
		"running": false, "state": "completed",
	})
	commandFinalMeta = testMeta.WithTerminalInfo(commandFinalMeta, "terminal-1")
	commandFinalMeta = testMeta.WithTerminalExit(commandFinalMeta, "terminal-1", &exitCode, nil)
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", TurnID: "turn-1", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCallUpdate{
			SessionUpdate: eventstream.UpdateToolCallInfo, ToolCallID: "command-1", Status: &completed, Meta: commandFinalMeta,
		},
	})
	...
	taskMeta := testMeta.WithRuntimeSection(acpToolNameMeta("Task"), testMeta.RuntimeTool, map[string]any{
		testMeta.RuntimeToolName: "Task", "action": "wait",
		"target_id": "task-1", "target_kind": "command",
	})
	taskMeta = testMeta.WithRuntimeSection(taskMeta, testMeta.RuntimeTask, map[string]any{
		"task_id": "task-1", testMeta.RuntimeTaskTerminalID: "terminal-1",
		"running": false, "state": "completed",
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCall{
			SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "task-wait-1",
			Title: "Task wait task-1", Kind: eventstream.ToolKindOther, Status: eventstream.ToolStatusInProgress,
			RawInput: taskInput, Meta: taskMeta,
		},
	})
...
func TestTaskReadUsesActivityHintAndFoldsObservationIntoCommandOwner(t *testing.T) {
	...
	applyStream := func(update eventstream.Update, sequence uint64) {
		model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
	...
	firstMeta = testMeta.WithRuntimeSection(firstMeta, testMeta.RuntimeTask, map[string]any{
		"handle": "command-3",
	})
	firstMeta = testMeta.WithRuntimeSection(firstMeta, testMeta.RuntimeStream, map[string]any{
		testMeta.RuntimeStreamMode:   "append",
		testMeta.RuntimeOutputCursor: int64(len([]byte(first))),
	})
	...
	readMeta := testMeta.WithRuntimeSection(acpToolNameMeta("Task"), testMeta.RuntimeTool, map[string]any{
		testMeta.RuntimeToolName: "Task", "action": "read",
		"target_handle": "command-3", "target_kind": "command",
	})
	readMeta = testMeta.WithRuntimeSection(readMeta, testMeta.RuntimeTask, map[string]any{
		...
		testMeta.RuntimeOutputStart:  int64(len([]byte(first))),
		testMeta.RuntimeOutputCursor: int64(len([]byte(first + second))),
		testMeta.RuntimeOutputDelta:  second,
		"running":                    true, "state": "running", "handle": "command-3",
	})
	apply(eventstream.ToolCall{
		SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "task-read",
		Title: "Task read command-3", Kind: eventstream.ToolKindOther, Status: eventstream.ToolStatusInProgress,
	...
	terminalReadMeta := testMeta.WithRuntimeSection(acpToolNameMeta("Task"), testMeta.RuntimeTool, map[string]any{
		testMeta.RuntimeToolName: "Task", "action": "read",
		"target_handle": "command-3", "target_kind": "command",
	})
	apply(eventstream.ToolCall{
		SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "task-read-terminal",
		Title: "Task read command-3", Kind: eventstream.ToolKindOther, Status: eventstream.ToolStatusInProgress,
...
func TestHandleACPEventEnvelopeMergesGrokGlobUpdate(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCallUpdate{
			...
			Kind:          stringPtr(eventstream.ToolKindSearch),
	...
	event := block.Events[0]
	if event.Name != "" || event.ToolKind != eventstream.ToolKindSearch || event.Title != "Glob `**/*.py`" || event.Args != "**/*.py" {
...
func TestHandleACPEventEnvelopeMergesGrokShellUpdate(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCallUpdate{
			...
			Content: []eventstream.ToolCallContent{{
				Type: "content", Content: eventstream.TextContent{Type: "text", Text: finalOutput},
			}},
		},
	})
	...
	event := block.Events[0]
	if event.Name != "" || event.Args != "pwd" || event.ToolKind != eventstream.ToolKindExecute || event.Title != "Execute `pwd`" || !event.Done || !strings.Contains(event.Output, strings.TrimSpace(finalOutput)) {
...
func TestProjectedGrokListUpdateKeepsOtherKindOutOfExactToolName(t *testing.T) {
	...
	if event := block.Events[0]; event.Name != "" || event.ToolKind != eventstream.ToolKindOther || event.Title != "List `"+path+"`" {
...
func TestNormalizedGrokListJoinsExploredWithoutExactToolName(t *testing.T) {
	...
	tools := []struct {
		...
		{callID: "read-1", title: "Read `go.mod`", kind: eventstream.ToolKindRead, input: map[string]any{"target_file": "go.mod"}},
	...
	for _, tool := range tools {
		...
		model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
			Kind: eventstream.KindSessionUpdate, SessionID: "session-1", Scope: eventstream.ScopeMain,
			Update: eventstream.ToolCallUpdate{
				SessionUpdate: eventstream.UpdateToolCallInfo, ToolCallID: tool.callID,
				Status: stringPtr(eventstream.ToolStatusCompleted),
			},
		})
	}
	...
	list := block.Events[0]
	if list.Name != "" || list.ToolKind != eventstream.ToolKindRead || list.ExplorationVerb != "List" || list.Args != "docs" || !list.Done {
...
func TestNormalizedGrokLiveOtherMatchesLoadedListPresentation(t *testing.T) {
	...
	settle := func(t *testing.T, model *Model) (SubagentEvent, string) {
		...
		model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
			Kind: eventstream.KindSessionUpdate, SessionID: "session-1", Scope: eventstream.ScopeMain,
			Update: eventstream.ToolCall{
				SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "read-1", Title: "Read `go.mod`",
				Kind: eventstream.ToolKindRead, Status: eventstream.ToolStatusCompleted,
	...
	if liveList.ToolKind != eventstream.ToolKindOther || loadedList.ToolKind != eventstream.ToolKindRead {
...
func TestAnonymousProviderTitleRemainsAnAtomicLifecycleLabel(t *testing.T) {
	...
	got := standardToolLifecycleHeader(SubagentEvent{
		ToolKind: eventstream.ToolKindOther,
		Title:    title,
		Args:     "foo",
	}, false)
...
func TestHandleACPEventEnvelopeMergesPartialToolUpdateFromCaelisOutput(t *testing.T) {
	...
	event := block.Events[0]
	if event.Name != "" || event.ToolKind != eventstream.ToolKindExecute || event.Title != "RunCommand pwd" || event.Args != "pwd" || !event.Done {
...
func TestHandleACPEventEnvelopePreservesSparseExplorationContentAfterSettlement(t *testing.T) {
	...
	model := NewModel(Config{NoColor: true, NoAnimation: true})
	readKind := eventstream.ToolKindRead
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCallUpdate{
			...
			Status:        &inProgress,
			Content: []eventstream.ToolCallContent{{
				Type: "content", Content: eventstream.TextContent{Type: "text", Text: "first result"},
			}},
		},
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCallUpdate{
			...
			ToolCallID:    "read-1",
			Content: []eventstream.ToolCallContent{{
				Type: "content", Content: eventstream.TextContent{Type: "text", Text: " late detail"},
			}},
		},
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCallUpdate{
			...
			Status:        &inProgress,
			Content: []eventstream.ToolCallContent{{
				Type: "content", Content: eventstream.TextContent{Type: "text", Text: " stale detail"},
			}},
		},
	})
	...
	for _, text := range []string{"[1/2] 14:57:57\n", "[2/2] 14:57:58\n"} {
		model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
			...
			Update: eventstream.ToolCallUpdate{
				SessionUpdate: eventstream.UpdateToolCallInfo,
				ToolCallID:    "call-1",
				Meta:          testMeta.WithTerminalOutput(acpToolNameMeta("RunCommand"), "call-1", text),
			},
		})
	}
...
func TestHandleACPEventEnvelopeAppliesParticipantSequence(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			...
			Content:       eventstream.TextContent{Type: "text", Text: "participant answer"},
		},
	})
...
func TestHandleACPEventEnvelopeShowsParticipantFailureBeforeTerminal(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			...
			Content:       eventstream.TextContent{Type: "text", Text: "inspect the change"},
		},
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
...
func TestParticipantLiveAndFinalWithoutMessageIDRenderExactlyOnce(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateUserMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "introduce yourself"},
		},
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind:          eventstream.KindSessionUpdate,
...
func TestHandleACPEventEnvelopeKeepsParticipantSkillContentAsStandardRead(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCallUpdate{
			...
			Kind:          stringPtr(eventstream.ToolKindRead),
			Status:        stringPtr(eventstream.ToolStatusCompleted),
		},
	})
...
func TestHandleACPEventEnvelopeAnchorsSubagentOutputToSpawnTool(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "subagent found the issue"},
		},
	})
	...
	for index, chunk := range chunks {
		model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
			...
			Update: eventstream.ContentChunk{
				...
				Content:       eventstream.TextContent{Type: "text", Text: chunk.text},
			},
		})
...
func TestHandleACPEventEnvelopePreservesHiddenChildToolAsBlankMessageBoundary(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, childEnvelope(eventstream.ContentChunk{
		SessionUpdate: eventstream.UpdateAgentMessage,
		Content:       eventstream.TextContent{Type: "text", Text: "任务 3 完成。\n---"},
	}))
	model = applyACPEnvelopeForTest(t, model, childEnvelope(eventstream.ToolCall{
		SessionUpdate: eventstream.UpdateToolCall, ToolCallID: "child-tool-1",
		Title: "Write", Kind: eventstream.ToolKindEdit, Status: eventstream.ToolStatusInProgress,
	}))
	model = applyACPEnvelopeForTest(t, model, childEnvelope(eventstream.ContentChunk{
		SessionUpdate: eventstream.UpdateAgentMessage,
		Content:       eventstream.TextContent{Type: "text", Text: "### 任务 4：创建文件"},
	}))
...
func TestHandleACPEventEnvelopeShowsChildToolActivityInRunningSpawn(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ToolCallUpdate{
			...
			Content: []eventstream.ToolCallContent{{
				Type: "content", Content: eventstream.TextContent{Type: "text", Text: "loaded child settings"},
			}},
		},
	})
	overlay = subagentOutputOverlayPlain(model)
...
func TestHandleACPEventEnvelopeRoutesCrossTurnChildContinuationAfterSendMessage(t *testing.T) {
	...
	completed := eventstream.ToolStatusCompleted
	spawnFinalMeta := testMeta.WithRuntimeSection(spawnMeta, testMeta.RuntimeTask, map[string]any{
		"task_id": "task-1",
		"handle":  "task-1",
		"running": false,
		"state":   "completed",
...
func TestHandleACPEventEnvelopeRendersSemanticSpawnEventsOnce(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			...
			Content:       eventstream.TextContent{Type: "text", Text: "child private thought"},
		},
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind:      eventstream.KindSessionUpdate,
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "child semantic summary\n"},
		},
	})
...
func TestHandleACPEventEnvelopeKeepsTaskWriteCompactWhenChildIsAnchoredToIt(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "child write response"},
		},
	})
...
func TestHandleACPEventEnvelopeAppliesSpawnFinalRuntimeResultWithoutTerminalOutput(t *testing.T) {
	...
	spawnMeta = testMeta.WithRuntimeSection(spawnMeta, testMeta.RuntimeTask, map[string]any{
		"task_id":                      "task-1",
		testMeta.RuntimeTaskTerminalID: "subagent-task-1",
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind:      eventstream.KindSessionUpdate,
		SessionID: "session-1",
	...
	completed := eventstream.ToolStatusCompleted
	finalMeta := testMeta.WithRuntimeSection(spawnMeta, testMeta.RuntimeTask, map[string]any{
		"task_id":                      "task-1",
		testMeta.RuntimeTaskTerminalID: "subagent-task-1",
		"running":                      false,
		"state":                        "completed",
...
func TestForwardTurnEventStreamQueuesLiveACPEnvelopes(t *testing.T) {
	...
	events <- eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "live answer"},
		},
	}
	events <- terminal
	close(events)
...
func TestParticipantFooterSuppressesOuterTurnDivider(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentThought,
			Content:       eventstream.TextContent{Type: "text", Text: "checking"},
		},
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind:          eventstream.KindSessionUpdate,
...
func TestHandleACPEventEnvelopeSuppressesLateParticipantUserEchoAfterTurnStarts(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "checking weather"},
		},
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind:          eventstream.KindSessionUpdate,
...
func TestHandleACPEventEnvelopeSuppressesLateParticipantUserEchoAfterTurnCompletes(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "weather result"},
		},
	})
...
func TestAcceptedActiveTurnPromptDisplaysBeforeFollowupMainOutput(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "before guidance"},
		},
	})
	model.pendingQueue = append(model.pendingQueue, pendingPrompt{
		execLine:    prompt,
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "after guidance"},
		},
	})
...
func TestActiveTurnPromptRendersAtLLMStepInsertionEcho(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "I will search next."},
		},
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind:      eventstream.KindSessionUpdate,
...
func TestHandleACPEventEnvelopeShowsContextCompactingHintUntilSuccessNotice(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentThought,
			Content:       eventstream.TextContent{Type: "text", Text: "Checking the remaining context."},
		},
	})
...
func TestHandleACPEventEnvelopeKeepsCompactionForegroundAcrossBackgroundCommandStream(t *testing.T) {
	...
	backgroundFrame := func(status *string, meta map[string]any, sequence uint64) eventstream.Envelope {
		return eventstream.Envelope{
			...
			Update: eventstream.ToolCallUpdate{
				SessionUpdate: eventstream.UpdateToolCallInfo, ToolCallID: "command-1", Status: status, Meta: meta,
			},
		}
	}
		...
		},
		{
			...
			env: eventstream.Envelope{
				...
				Permission: &eventstream.RequestPermissionRequest{
					ToolCall: eventstream.ToolCallUpdate{SessionUpdate: eventstream.UpdateToolCallInfo, ToolCallID: "call-1"},
				},
			},
			lane: renderLaneToolStream,
		},
	...
	cases := []struct {
	...
	}{
		{
			...
			replay: []eventstream.Envelope{
				{
					...
					Update: eventstream.ContentChunk{
						SessionUpdate: eventstream.UpdateAgentMessage,
						Content:       eventstream.TextContent{Type: "text", Text: "interrupted output"},
					},
				},
				{
					Kind:      eventstream.KindLifecycle,
			...
			live: eventstream.Envelope{
				...
				Update: eventstream.ContentChunk{
					SessionUpdate: eventstream.UpdateAgentMessage,
					Content:       eventstream.TextContent{Type: "text", Text: "new answer"},
				},
			},
		...
		},
		{
			...
			replay: []eventstream.Envelope{
				{
					...
					Update: eventstream.ContentChunk{
						SessionUpdate: eventstream.UpdateAgentMessage,
						Content:       eventstream.TextContent{Type: "text", Text: "old async bash summary"},
					},
				},
			},
			...
			live: eventstream.Envelope{
				...
				Update: eventstream.ContentChunk{
					SessionUpdate: eventstream.UpdateAgentMessage,
					Content:       eventstream.TextContent{Type: "text", Text: "new replay check answer"},
				},
			},
...
func TestMainACPEventWithoutTurnIDDoesNotReuseSessionBlock(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "old answer"},
		},
	})
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "missing turn answer"},
		},
	})
...
func TestMainACPKeyedThenKeylessEventsStayInOneLiveTurnBlock(t *testing.T) {
	...
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "keyed chunk"},
		},
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind:      eventstream.KindSessionUpdate,
		...
		Update: eventstream.ContentChunk{
			SessionUpdate: eventstream.UpdateAgentMessage,
			Content:       eventstream.TextContent{Type: "text", Text: "keyless chunk"},
		},
	})
...
func TestMainTimelineRoutesCrossTurnTaskObserverStreamToOriginalCommandPanel(t *testing.T) {
	...
	taskMeta := testMeta.WithRuntimeSection(acpToolNameMeta("Task"), testMeta.RuntimeTool, map[string]any{
		testMeta.RuntimeToolName: "Task", "action": "wait",
		"target_id": "task-1", "target_kind": "terminal",
	})
	model = applyACPEnvelopeForTest(t, model, eventstream.Envelope{
		Kind: eventstream.KindSessionUpdate, SessionID: "session-1", TurnID: "turn-2", Scope: eventstream.ScopeMain,
		Update: eventstream.ToolCall{
...
func commandTaskStreamEnvelopeForTest(sessionID string, callID string, taskHandle string, output string) eventstream.Envelope {
	streamMeta := map[string]any{
		testMeta.RuntimeStreamMode:   "append",
		testMeta.RuntimeOutputCursor: int64(len([]byte(output))),
	}
	meta := testMeta.WithCompactRuntimeSection(nil, testMeta.RuntimeStream, streamMeta)
	toolMeta := map[string]any{testMeta.RuntimeToolName: "RunCommand"}
	if taskHandle != "" {
		toolMeta["target_handle"] = taskHandle
	}
	meta = testMeta.WithRuntimeSection(meta, testMeta.RuntimeTool, toolMeta)
	meta = testMeta.WithTerminalOutput(meta, callID, output)
	meta = testMeta.WithTerminalInfo(meta, callID)
	status := eventstream.ToolStatusInProgress
	return eventstream.Envelope{
		Kind:      eventstream.KindSessionUpdate,
		SessionID: sessionID,
		...
		Update: eventstream.ToolCallUpdate{
			...
			ToolCallID:    callID,
			Status:        &status,
			Content:       []eventstream.ToolCallContent{{Type: "terminal", TerminalID: callID}},
			Meta:          meta,
		},
	}
}
