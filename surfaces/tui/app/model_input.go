...
func (m *Model) handleKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case key.Matches(msg, m.keys.Send):
		if m.mentionCompletionNeedsResolution() {
			return m, m.requestMentionCompletion(0)
		}
...
func (m *Model) submitInteractiveLine(execLine string, displayLine string, attachments []Attachment) (tea.Model, tea.Cmd) {
	...
	if opened, cmd := m.tryOpenSlashArgPicker(execLine); opened {
		return m, cmd
	}
	return m.submitLineWithDisplayAndAttachmentsOptions(execLine, displayLine, attachments, submitLineOptions{
		recordHistory: true,
...
func (m *Model) resolveSubmission(execLine string, alreadyRunning bool) resolvedSubmission {
	...
	canSubmitNow := true
	if alreadyRunning && uiMode != SubmissionModeOverlay && !isSessionSelectionLine(execLine) {
...
func (m *Model) tryToggleFoldToken(blockID string, token string) bool {
	...
	case *ParticipantTurnBlock:
		handle := taskStreamPanelHandle(blk.Events, callID)
