...
func (m *Model) buildHintText() string {
	...
	if m.mentionRequestPending || m.slashArgRequestPending {
		return m.theme.HelpHintTextStyle().Render("loading completions...")
	}
...
func (m *Model) footerYoloText() string {
	if m == nil {
		return ""
	}
	if m.cfg.FullAccessMode {
		return statusbar.FooterYoloLabel
	}
	return m.statusView.FooterYoloText()
}
...
func (m *Model) footerParts(width int) (left string, subagents string, right string) {
	yolo := m.footerYoloText()
	fitWidth := width
	if yolo != "" {
		reserved := displayColumns(yolo)
		if width > reserved+displayColumns(" · ") {
			reserved += displayColumns(" · ")
		}
		fitWidth = maxInt(0, width-reserved)
	}
	left, subagents, right = fitStatusFooterParts(
		fitWidth,
		m.footerIdentityText(),
		m.footerSubagentText(),
		m.footerSubagentCompactText(),
		m.footerContextText(),
	)
	return joinFooterSegments(yolo, left), subagents, right
}

func joinFooterSegments(left string, right string) string {
...
func styleFooterLeft(m *Model, plain string) string {
	...
	sep := " · "
	var prefix string
	if remainder, ok := strings.CutPrefix(plain, statusbar.FooterYoloLabel); ok {
		prefix = m.theme.WarnStyle().Bold(true).Render(statusbar.FooterYoloLabel)
		if remainder == "" {
			return prefix
		}
		plain = strings.TrimPrefix(remainder, sep)
		if plain == "" {
			return prefix
		}
		prefix += m.theme.MutedTextStyle().Render(sep)
	}
	...
	if idx != -1 {
		...
		return prefix + styledModel + styledSep + styledWorkspace
	}
