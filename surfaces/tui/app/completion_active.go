...
func (m *Model) loadMoreCompletion(kind completionKind) bool {
	return kind == completionMention && m.loadMoreMentionCandidates()
}
...
func (m *Model) moveActiveCompletionSelection(delta int, wrap bool) {
	if delta == 0 {
		return
	}
	snapshot, geometry, ok := m.activeCompletionGeometry()
	...
	if delta > 0 && snapshot.selected >= snapshot.total-1 && snapshot.canLoadMore {
		oldTotal := snapshot.total
		if m.loadMoreCompletion(snapshot.kind) {
			if refreshed, ok := m.completionSnapshot(); ok &&
				refreshed.kind == snapshot.kind &&
				refreshed.total > oldTotal {
				m.setCompletionIndex(refreshed.kind, oldTotal)
