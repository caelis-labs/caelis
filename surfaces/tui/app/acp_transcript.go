...
func renderACPTranscriptHeaderRow(blockID string, plain string, width int, ctx BlockRenderContext, token string) RenderedRow {
	return renderACPTranscriptHeaderRowMarked(blockID, plain, width, ctx, token, acpHeaderMarkDefault, false)
}

func renderACPToolHeaderRow(blockID string, plain string, width int, ctx BlockRenderContext, token string, err bool, completed bool) RenderedRow {
	tone, dim := acpToolHeaderMark(ctx, err, completed)
	return renderACPTranscriptHeaderRowMarked(blockID, plain, width, ctx, token, tone, dim)
}

func renderACPTranscriptHeaderRowMarked(
	blockID string,
	plain string,
	width int,
	ctx BlockRenderContext,
	...
	styled := styleACPTranscriptHeaderWithMark(ctx, plain, tone, dim)
...
func acpToolHeaderMark(ctx BlockRenderContext, err bool, completed bool) (acpHeaderMarkTone, bool) {
	if err {
		return acpHeaderMarkDanger, false
	}
	if !completed && ctx.AnimationsEnabled {
		return acpHeaderMarkDefault, subagentOutputPulseDim(ctx.SpinnerView)
	}
...
func styleACPTranscriptHeaderDetail(ctx BlockRenderContext, verb string, detail string) string {
	...
	if path, added, removed, ok := tuikit.SplitDiffCountTokens(detail); ok {
		styled := tuikit.StyleDiffCountTokens(added, removed, ctx.Theme)
		if path == "" {
			return styled
		}
		return styleToolDetailNumbers(path, ctx, ctx.Theme.ToolArgsStyle()) + " " + styled
	}
