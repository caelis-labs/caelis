...
// Live reasoning has one height owner:
//
//   - the current tail (still streaming, or completed with no later step) uses a
//     fixed 5-line preview window
//   - once this reasoning is settled and later work exists, it folds to one
//     › summary row
//   - the next exploration step absorbs the previous stage into Explored
//
// Tools in the same live step are not a reason to collapse a still-active tail,
// so the temporary step cannot bounce 5 → 1 → 5 before it enters Explored.
...
func reasoningShouldFold(events []SubagentEvent, idx int, status string) bool {
	...
	// Keep the 5-line window while this reasoning is still the active tail.
	// A later fold boundary only collapses settled reasoning, so the first
	// exploration tool cannot flash a 1-line summary that the second tool
	// would immediately reopen.
	if participantNarrativeEventActive(events, end, status) {
		return false
	}
...
func reasoningFoldBoundaryEvent(ev SubagentEvent) bool {
	...
	case SENotice:
		// Durable notices settle prior layout. Model-retry banners are transient
...
// liveTailHasPotentialDeferredCompactStage is owned by compactHeightBudget.
// It pads the block when a 2+ tool exploration tail is about to merge into
// Explored; it must not drive reasoning fold policy.
