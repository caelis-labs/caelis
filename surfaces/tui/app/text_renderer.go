...
func protectActiveNarrativeRepaintRows(rows []RenderedRow, width int) []RenderedRow {
	...
	for i := range rows {
		...
		// Guard every live tail, including ASCII. A near-full English line that
		// later grows uses the same ICH path and can shove the last word onto
		// the following CJK row on Ghostty.
		rows[i].Styled = protectRepaintGuardLine(rows[i].Styled, width)
	}
	return rows
}
