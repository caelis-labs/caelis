...
func slashPluginWithContext(ctx context.Context, service controlprompt.PluginService, send func(tea.Msg), args string) TaskResultMsg {
	...
	switch sub {
	case "":
		sendNotice(send, pluginUsageText(), SlashNoticeHint)
	...
	case "manage":
		...
		if len(plugins) == 0 {
			sendNotice(send, "No plugins installed", SlashNoticeFeedback)
			return TaskResultMsg{SuppressTurnDivider: true}
		}
	...
	case "install":
		...
		if target == "" {
			sendNotice(send, "usage: /plugin install <plugin@marketplace|directory-path>", SlashNoticeHint)
			return TaskResultMsg{SuppressTurnDivider: true}
		}
		...
		sendNotice(send, fmt.Sprintf("Installed plugin %s", firstNonEmpty(strings.TrimSpace(p.ID), strings.TrimSpace(p.Name))), SlashNoticeFeedback)
		sendNotice(send, "Restart new tasks to apply the plugin", SlashNoticeHint)
		return TaskResultMsg{SuppressTurnDivider: true}
	case "marketplace":
	...
	case "rm":
		target := strings.TrimSpace(rest)
		if target == "" {
			sendNotice(send, "usage: /plugin rm <plugin-id>", SlashNoticeHint)
			return TaskResultMsg{SuppressTurnDivider: true}
		}
		...
		sendNotice(send, fmt.Sprintf("Removed plugin %s", target), SlashNoticeFeedback)
		sendNotice(send, "Restart new tasks to apply the change", SlashNoticeHint)
		return TaskResultMsg{SuppressTurnDivider: true}
	default:
		sendNotice(send, pluginUsageText(), SlashNoticeHint)
		return TaskResultMsg{SuppressTurnDivider: true}
...
func awaitPluginManagerSelection(ctx context.Context, service controlprompt.PluginService, send func(tea.Msg), plugins []controlprompt.PluginSnapshot, responses <-chan PromptResponse) {
	...
	for _, p := range plugins {
		...
		case wantEnabled && !p.Enabled:
			if _, err := service.EnablePlugin(ctx, id); err != nil {
				sendNotice(send, fmt.Sprintf("Could not enable %s", id), SlashNoticeFeedback)
				return
			}
			enabled = append(enabled, id)
		case !wantEnabled && p.Enabled:
			if _, err := service.DisablePlugin(ctx, id); err != nil {
				sendNotice(send, fmt.Sprintf("Could not disable %s", id), SlashNoticeFeedback)
				return
			}
	...
	if len(enabled) == 0 && len(disabled) == 0 {
		sendNotice(send, "Plugin selection unchanged", SlashNoticeHint)
		return
	}
	...
	sendNotice(send, "Updated plugins: "+strings.Join(parts, "; "), SlashNoticeFeedback)
	sendNotice(send, "Restart new tasks to apply the change", SlashNoticeHint)
}
