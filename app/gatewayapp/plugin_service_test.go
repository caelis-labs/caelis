...
func TestPluginServiceAddPathAffectsFutureSessionSkillPrompt(t *testing.T) {
	tmp := t.TempDir()
	storeDir := filepath.Join(tmp, "store")
	workspaceDir := filepath.Join(tmp, "ws")
	if err := os.MkdirAll(storeDir, 0o700); err != nil {
...
// TestPluginServiceEnableValidatesBeforeCAS verifies that a missing enabled
// plugin is rejected before configuration is committed.
func TestPluginServiceEnableValidatesBeforeCAS(t *testing.T) {
	tmp := t.TempDir()
	storeDir := filepath.Join(tmp, "store")
	workspaceDir := filepath.Join(tmp, "ws")
	if err := os.MkdirAll(storeDir, 0o700); err != nil {
	...
	// Sabotage the plugin directory so candidate validation fails before CAS.
	...
	if !strings.Contains(err.Error(), "parse enabled plugin") {
		t.Errorf("Enable() error = %q, want candidate validation failure", err.Error())
	}
...
func TestPluginServiceRemoveRejectsInvalidCandidateBeforeCAS(t *testing.T) {
	tmp := t.TempDir()
	storeDir := filepath.Join(tmp, "store")
	workspaceDir := filepath.Join(tmp, "ws")
	if err := os.MkdirAll(storeDir, 0o700); err != nil {
...
func TestPluginServiceDisableRejectsInvalidCandidateBeforeCAS(t *testing.T) {
	tmp := t.TempDir()
	storeDir := filepath.Join(tmp, "store")
	workspaceDir := filepath.Join(tmp, "ws")
	if err := os.MkdirAll(storeDir, 0o700); err != nil {
...
func TestPluginServiceCommitsWhileTurnIsActiveWithoutReplacingRuntime(t *testing.T) {
	ctx := context.Background()
...
func TestPluginServiceValidationFailureDoesNotEnterConfigWriter(t *testing.T) {
	tmp := t.TempDir()
	storeDir := filepath.Join(tmp, "store")
	workspaceDir := filepath.Join(tmp, "ws")
	if err := os.MkdirAll(storeDir, 0o700); err != nil {
	...
	if !strings.Contains(err.Error(), "parse enabled plugin") {
		t.Errorf("expected candidate validation failure, got: %v", err)
	}
	if saveCount != 0 {
		t.Errorf("validation failure entered config writer %d times", saveCount)
	}
}
...
func TestPluginServiceMCPServers(t *testing.T) {
	...
	if mcpSrv.Status != "stopped" {
		t.Errorf("expected current Host status 'stopped', got %s", mcpSrv.Status)
	}
...
func TestPluginServiceRejectsAgentCollisionBeforeCAS(t *testing.T) {
	tmp := t.TempDir()
	storeDir := filepath.Join(tmp, "store")
	workspaceDir := filepath.Join(tmp, "ws")
	pluginDir := filepath.Join(tmp, "agentplugin")
	...
	manifestJSON := fmt.Sprintf(`{
		"name": "agent-plugin",
		"version": "1.0.0",
		"agents": [{"name": "shared-helper", "command": %q}]
	}`, os.Args[0])
	buildMinimalPluginDir(t, pluginDir, manifestJSON)
	...
	stack, err := NewLocalStack(Config{
		...
		Assembly: assembly.ResolvedAssembly{Agents: []assembly.AgentConfig{{
			Name: "shared-helper", Command: os.Args[0],
		}}},
	})
	if err != nil {
		t.Fatalf("NewLocalStack() error = %v", err)
	...
	if len(doc.Plugins) != 0 {
		t.Fatalf("plugins after rejected candidate = %#v, want unchanged configuration", doc.Plugins)
	}
}
