...
func (s *runtimeComposition) doctorForWorkspace(ctx context.Context, workspace session.WorkspaceRef, req DoctorRequest) (DoctorReport, error) {
	...
	if alias == "" {
		alias = strings.TrimSpace(s.EffectiveModelAlias())
		modelRef = alias
	}
	...
	sandbox := s.SandboxStatusForWorkspace(workspace)
	...
	if report.FullAccessMode {
		report.Warnings = append(report.Warnings, "YOLO host escape is active")
	...
	report.Warnings = append(report.Warnings, s.legacyPluginSkillCopyWarnings(workspace.CWD)...)
