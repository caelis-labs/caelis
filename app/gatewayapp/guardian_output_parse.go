...
// Guardian assessments are deliberately small: even the option-bearing form
// has five short fields and a rationale capped at 160 runes. Keep text-mode
// envelope recovery bounded independently of provider output-token support.
...
func normalizeGuardianAssessment(parsed guardianReviewModelOutput, options []kernel.ApprovalOption) (guardianReviewModelOutput, error) {
	...
	if len([]rune(parsed.Rationale)) > 160 {
		return guardianReviewModelOutput{}, fmt.Errorf("approval reviewer rationale exceeds 160 characters")
	}
