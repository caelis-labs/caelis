...
func TestAuthenticateModelProviderRoutesGrokOAuthWithoutCatalogRead(t *testing.T) {
	client := &http.Client{Transport: grokOAuthHandlerTransport{handler: http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case request.URL.Host == "cli-chat-proxy.grok.com" && request.URL.Path == "/v1/models":
			t.Error("authentication unexpectedly read the account model catalog")
			http.Error(writer, "unexpected catalog read", http.StatusInternalServerError)
		default:
	...
	stack := &Stack{runtimeComposition: runtimeComposition{grokAuth: manager}}
...
func TestGrok45AdvertisesImagePromptCapability(t *testing.T) {
	...
	if !modelConfigSupportsImages(ModelConfig{Provider: "xai", Model: "grok-4.6"}) {
		t.Fatal("modelConfigSupportsImages(xai/grok-4.6) = false, want true")
	}
}
