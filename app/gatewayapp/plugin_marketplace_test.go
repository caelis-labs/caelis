...
func TestDiscoverOpenCodeFindsLocalPluginsAndNPMPackages(t *testing.T) {
	t.Parallel()

	tmp := t.TempDir()
	pluginDir := filepath.Join(tmp, ".opencode", "plugins")
	if err := os.MkdirAll(pluginDir, 0o700); err != nil {
		t.Fatalf("mkdir plugin dir: %v", err)
	}
	if err := os.WriteFile(filepath.Join(pluginDir, "notify.ts"), []byte("export const Plugin = async () => ({})"), 0o600); err != nil {
		t.Fatalf("write plugin file: %v", err)
	}
	if err := os.WriteFile(filepath.Join(tmp, "opencode.json"), []byte(`{"plugin":["opencode-helicone-session"]}`), 0o600); err != nil {
		t.Fatalf("write opencode config: %v", err)
	}

	stack := buildPluginStack(t, filepath.Join(tmp, "store"), tmp)
	discovery, err := stack.Plugins().DiscoverOpenCode(tmp)
	if err != nil {
		t.Fatalf("DiscoverOpenCode() error = %v", err)
	}
	if len(discovery.LocalPlugins) != 1 || discovery.LocalPlugins[0].Name != "notify" {
		t.Fatalf("DiscoverOpenCode() local = %#v, want notify.ts", discovery.LocalPlugins)
	}
	if len(discovery.NPMPackages) != 1 || discovery.NPMPackages[0].Package != "opencode-helicone-session" {
		t.Fatalf("DiscoverOpenCode() npm = %#v, want npm package from opencode.json", discovery.NPMPackages)
	}
}

func TestImportOpenCodeRegistersUnsupportedLocalPlugin(t *testing.T) {
	t.Parallel()
	...
	if err := os.WriteFile(filepath.Join(pluginDir, "guard.js"), []byte("export const Plugin = async () => ({})"), 0o600); err != nil {
		t.Fatalf("write plugin file: %v", err)
	}

	stack := buildPluginStack(t, filepath.Join(tmp, "store"), tmp)
	infos, err := stack.Plugins().ImportOpenCode(context.Background(), tmp)
	if err != nil {
		t.Fatalf("ImportOpenCode() error = %v", err)
	}
	if len(infos) != 1 || infos[0].Status != "unsupported" || !strings.Contains(infos[0].Warning, "cannot be executed") {
		t.Fatalf("ImportOpenCode() = %#v, want unsupported local plugin with warning", infos)
	}

	listed, err := stack.Plugins().List(context.Background())
	if err != nil {
		t.Fatalf("List() error = %v", err)
	}
	if len(listed) != 1 || listed[0].Status != "unsupported" || !strings.Contains(listed[0].Warning, "cannot be executed") {
		t.Fatalf("List() = %#v, want persisted unsupported local plugin with warning", listed)
	}

	inspected, err := stack.Plugins().Inspect(context.Background(), "opencode-guard")
	if err != nil {
		t.Fatalf("Inspect() error = %v", err)
	}
	if inspected.Status != "unsupported" || !strings.Contains(inspected.Warning, "cannot be executed") {
		t.Fatalf("Inspect() = %#v, want persisted unsupported local plugin with warning", inspected)
	}
}
