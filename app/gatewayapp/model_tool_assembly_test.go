...
func TestProductAgentFinalModelRequestIncludesCanonicalTaskCommunicationTools(t *testing.T) {
	...
	description, _ := sendFunction["description"].(string)
	for _, want := range []string{"incremental update or question", "terminal answer in the final response", "routing boundary owns delivery"} {
		if !strings.Contains(description, want) {
			t.Fatalf("final SendMessage description missing %q: %q", want, description)
		}
	}
}
