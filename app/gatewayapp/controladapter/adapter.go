...
func (d *Adapter) status(ctx context.Context, includeDiagnostics bool) (StatusSnapshot, error) {
	...
	status := StatusSnapshot{
		...
		SandboxSetup:                    sandboxSetupStatusFromPort(sandboxStatus.Setup),
	...
	if d.stack != nil {
		...
		if includeDiagnostics {
			if report, err := d.stack.Doctor(context.Background(), req); err == nil {
				...
				if report.SandboxSetup != nil {
					status.SandboxSetup = sandboxSetupStatusFromPort(*report.SandboxSetup)
				}
		...
		if ok {
			if usage, err := d.sessionTokenUsageBreakdown(context.Background(), activeSession.SessionRef); err == nil {
				status.SessionUsageTotal = usageSnapshotFromKernel(usage.Total)
