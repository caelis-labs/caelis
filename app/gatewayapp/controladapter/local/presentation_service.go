...
func NewPresentationService(host *gatewayapp.Stack, modes acp.ModeProvider, useModes bool, configs acp.ConfigProvider) (*PresentationService, error) {
	...
	modeWriteTarget := controlclient.PresentationModeTargetApproval
	if useModes && modes != nil {
		modeWriteTarget = controlclient.PresentationModeTargetApp
	}
...
func (s *PresentationService) PresentationSnapshot(ctx context.Context, principal controlclient.Principal, req controlclient.PresentationRequest) (controlclient.PresentationSnapshot, error) {
	...
	if active.Controller.Kind == session.ControllerKindACP {
		...
		if statusErr != nil {
			return controlclient.PresentationSnapshot{}, statusErr
		}
		if found && len(remote.ModeOptions) > 0 {
			...
			modeWriteTarget = controlclient.PresentationModeTargetController
		}
	}
...
func presentationSnapshot(
	...
	modeWriteTarget string,
) controlclient.PresentationSnapshot {
	result := controlclient.PresentationSnapshot{ConfigOptions: presentationConfigOptions(configs)}
	if modes != nil {
		result.Modes = &controlclient.PresentationModeState{Target: strings.TrimSpace(modeWriteTarget), CurrentModeID: modes.CurrentModeID}
