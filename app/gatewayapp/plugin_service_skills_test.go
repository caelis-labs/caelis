...
func TestPluginServiceSkillsFollowEnabledStateAndSuppressLegacyCopies(t *testing.T) {
	...
	enabled := activateFutureAssemblyStack(t, stack, "plugin-skill-enabled-with-legacy")
	...
	discovered, err := enabled.Skills().Discover(ctx, workspaceDir)
	if err != nil {
	...
	report, err := enabled.Doctor(ctx, DoctorRequest{})
	if err != nil {
	...
	disabled := activateFutureAssemblyStack(t, stack, "plugin-skill-disabled-with-legacy")
	...
	discovered, err = disabled.Skills().Discover(ctx, workspaceDir)
	if err != nil {
