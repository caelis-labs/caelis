...
func TestGuardianConversationManagerForksSameStepAndJoinsInToolCallOrder(t *testing.T) {
	manager := newGuardianConversationManager()
	baseUser, baseAssistant := guardianConversationTestPair("base")
	committed, _, err := manager.commitValidated(guardianConversationCommit{
		SessionID:       "main-1",
		ExpectedVersion: 0,
		ParentCursor:    guardianParentCanonicalCursor{EventID: "parent-1", EventSeq: 1},
	...
	if err != nil || !committed {
		t.Fatalf("base commit = (%v, %v)", committed, err)
	}

	refs := []guardianConversationForkRef{
		{Key: "run-1\x00turn-1\x00step-2", Index: 0, CallCount: 2},
		{Key: "run-1\x00turn-1\x00step-2", Index: 1, CallCount: 2},
	}
	bases := make([]guardianConversationSnapshot, len(refs))
	for index, ref := range refs {
		bases[index], err = manager.fork("main-1", ref, nil)
		if err != nil {
			t.Fatal(err)
		}
		if bases[index].Version != 1 || strings.Join(guardianConversationTestTexts(bases[index].Events), ",") != "user-base,assistant-base" {
			t.Fatalf("fork %d base = %#v", index, bases[index])
		}
	}
	...
	results := make(chan bool, 2)
	errs := make(chan error, 2)
	for index, suffix := range []string{"zero", "one"} {
		index, suffix := index, suffix
		wg.Add(1)
		go func() {
			defer wg.Done()
			user, assistant := guardianConversationTestPair(suffix)
			joined, _, joinErr := manager.commitValidated(guardianConversationCommit{
				SessionID:       "main-1",
				ExpectedVersion: bases[index].Version,
				Fork:            refs[index],
				ParentCursor:    guardianParentCanonicalCursor{EventID: "parent-2", EventSeq: 2},
				User:            user,
				Assistant:       assistant,
	...
	for joined := range results {
		if !joined {
			t.Fatal("same-step branch lost its join")
		}
	}

	joined, err := manager.snapshot("main-1")
	if err != nil {
		t.Fatal(err)
	}
	if joined.Version != 3 {
		t.Fatalf("joined version = %d, want base plus two branches", joined.Version)
	}
	if got, want := strings.Join(guardianConversationTestTexts(joined.Events), ","), "user-base,assistant-base,user-zero,assistant-zero,user-one,assistant-one"; got != want {
		t.Fatalf("joined context = %q, want %q", got, want)
	}

	next, err := manager.fork("main-1", guardianConversationForkRef{
		Key: "run-1\x00turn-1\x00step-3", Index: 0, CallCount: 2,
	}, nil)
	if err != nil {
		t.Fatal(err)
	}
	if next.Version != joined.Version || !reflect.DeepEqual(next.Events, joined.Events) {
		t.Fatal("next model step did not inherit the complete joined prefix")
	}
}
