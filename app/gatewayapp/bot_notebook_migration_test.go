...
// TestBotNotebookProvisionedOnceAtOwnerPrompt covers a Bot created before
// notebooks were universal. Its owner's first prompt provisions the private
...
// never replaced.
func TestBotNotebookProvisionedOnceAtOwnerPrompt(t *testing.T) {
	...
	value := createLegacyNotebookTestBot(t, stack, "provision")
	...
	root, err := bot.NotebookRoot(storeDir, value.ID)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := os.Lstat(root); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("legacy fixture already has a notebook: %v", err)
	}
	prompt := func(input string) map[string]any {
		...
		before := len(requests)
		requestsMu.Unlock()
		result, err := stack.commandBackend.ExecuteControlCommand(ctx, principal, appserver.ActionPrompt, appserver.PromptRequest{
	...
	first := prompt("first legacy input")
	...
	second := prompt("second legacy input")
	...
	if len(currentMessages) <= len(previousMessages) || !reflect.DeepEqual(previousMessages, currentMessages[:len(previousMessages)]) ||
		...
		t.Fatal("legacy provisioning or a later prompt rewrote the provider request prefix")
	}
	assertBotNotebookWireTools(t, second)
}

// TestBotNotebookProvisionFailureRejectsPrompt pins the failure semantics of the
...
func TestBotNotebookProvisionFailureRejectsPrompt(t *testing.T) {
...
// createLegacyNotebookTestBot persists the released Bot shape: an owner-scoped
// conversation with its configuration record but no notebook on disk.
func createLegacyNotebookTestBot(t *testing.T, stack *Stack, name string) bot.Bot {
	...
	if err != nil || value.Config != config || value.ID != id {
