...
func normalizeConfig(cfg Config) Config {
	...
	if cfg.ParentPID == nil {
		cfg.ParentPID = os.Getppid
	}
	if cfg.ProcessExists == nil {
		cfg.ProcessExists = processExists
	}
	return cfg
}
...
func (m *Manager) Update(ctx context.Context, opts UpdateOptions) (Result, error) {
	...
	// The lock is restamped with the launcher PID so a later retry can tell a
	// live npm install from an interrupted or rebooted Windows update. The
	// launcher receives the exact path in the atomic handoff plan and owns
