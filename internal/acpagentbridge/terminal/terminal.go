...
func (a LocalTerminalAdapter) Output(ctx context.Context, req TerminalOutputRequest) (TerminalOutputResponse, error) {
	if a.Streams == nil {
		return TerminalOutputResponse{}, fmt.Errorf("internal/acpagentbridge/terminal: stream service is required")
	}
...
func (a LocalTerminalAdapter) WaitForExit(ctx context.Context, req TerminalWaitForExitRequest) (TerminalWaitForExitResponse, error) {
	...
	if !ok || controller == nil {
		return TerminalWaitForExitResponse{}, fmt.Errorf("internal/acpagentbridge/terminal: terminal wait is unsupported")
	}
...
func (a LocalTerminalAdapter) Kill(ctx context.Context, req TerminalKillRequest) error {
	controller, ok := a.Streams.(stream.Controller)
	if !ok || controller == nil {
		return fmt.Errorf("internal/acpagentbridge/terminal: terminal kill is unsupported")
	}
...
func (a LocalTerminalAdapter) Release(ctx context.Context, req TerminalReleaseRequest) error {
	controller, ok := a.Streams.(stream.Controller)
	if !ok || controller == nil {
		return fmt.Errorf("internal/acpagentbridge/terminal: terminal release is unsupported")
	}
...
func RefFromEvent(event *session.Event) (stream.Ref, bool) {
	...
	for _, meta := range terminalEventMetadata(event) {
		if ref.TaskID == "" {
			ref.TaskID = firstNonEmpty(mapString(meta, "task_id"), runtimeTaskMetaValue(meta, metautil.RuntimeTaskID))
		}
		if ref.TerminalID == "" {
			ref.TerminalID = firstNonEmpty(mapString(meta, "terminal_id"), runtimeTaskMetaValue(meta, metautil.RuntimeTaskTerminalID))
		}
	}
	if ref.TerminalID == "" {
	...
	if update := session.ProtocolUpdateOf(event); update != nil {
		if ref.TaskID == "" {
			ref.TaskID = runtimeTaskMetaValue(update.Meta, metautil.RuntimeTaskID)
		}
		if ref.TerminalID == "" {
			ref.TerminalID = runtimeTaskMetaValue(update.Meta, metautil.RuntimeTaskTerminalID)
		}
		if ref.TerminalID == "" {
...
func runtimeTaskMetaValue(meta map[string]any, key string) string {
	return metautil.String(meta, metautil.Root, metautil.Runtime, metautil.RuntimeTask, key)
}
