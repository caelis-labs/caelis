...
func (m *Manager) SetControllerModel(ctx context.Context, req SetControllerModelRequest) (ControllerStatus, error) {
	...
	if !m.isActiveControllerRun(run) {
		return ControllerStatus{}, fmt.Errorf("internal/acpagentbridge/controller: controller run is no longer active")
	}
	run.mu.Lock()
	actualEpoch := strings.TrimSpace(run.binding.EpochID)
	run.mu.Unlock()
	if expected := strings.TrimSpace(req.ExpectedControllerEpoch); expected != "" && actualEpoch != expected {
		return ControllerStatus{}, fmt.Errorf("internal/acpagentbridge/controller: expected controller epoch %q, actual %q", expected, actualEpoch)
	}
...
func (r *controllerRun) setControllerModel(ctx context.Context, req SetControllerModelRequest, clock func() time.Time) (ControllerStatus, error) {
	...
	effectStarted := false
	effectError := func(err error) error {
		if err == nil || !effectStarted {
			return err
		}
		return &ConfigurationEffectError{Err: err}
	}
	...
	if testModel != "" {
		...
		if err != nil {
			return ControllerStatus{}, effectError(err)
		}
	...
	if effort != "" {
		...
		if hasEffortOption && effortOption != nil {
			...
			if !ok {
				return ControllerStatus{}, effectError(fmt.Errorf("internal/acpagentbridge/controller: reasoning effort %q is not declared by the controller", effort))
			}
			effectStarted = true
...
func (r *controllerRun) setControllerMode(ctx context.Context, req SetControllerModeRequest, clock func() time.Time) (ControllerStatus, error) {
	...
	if modeOption, hasModeOption := pickModeConfigOption(configOptions); hasModeOption && modeOption != nil && len(modeOption.Options) > 0 {
		...
		if err != nil {
			return ControllerStatus{}, &ConfigurationEffectError{Err: err}
		}
