...
func newPromptRecorder(response acpsdk.RequestPermissionResponse) *promptRecorder {
	return &promptRecorder{response: response}
}

func allowOncePermissionResponse() acpsdk.RequestPermissionResponse {
...
func (r *promptRecorder) RequestPermission(_ context.Context, _ acpsdk.RequestPermissionRequest) (acpsdk.RequestPermissionResponse, error) {
...
func TestRuntimeAgentConformanceReplayOrdering(t *testing.T) {
	...
	rec := newPromptRecorder(allowOncePermissionResponse())
...
func TestRuntimeAgentConformanceEmitsRunCommandToolCallBeforeTerminalUpdates(t *testing.T) {
	...
	if got, want := strings.Join(terminalOutputPayloads(rec.Notifications(), "call-shell"), ""), "acp-run-command-test"; got != want {
		t.Fatalf("RUN_COMMAND terminal output = %q, want one exact delivery %q; notifications = %#v", got, want, rec.Notifications())
	}
}
...
func TestRuntimeAgentConformanceForwardsAdjacentStreamChunksVerbatim(t *testing.T) {
	...
	if got, want := agentMessageTexts(rec.Notifications()), []string{
		"Pipeline is running. Let me cancel it.",
		"Pipeline is running. Let me cancel it.",
