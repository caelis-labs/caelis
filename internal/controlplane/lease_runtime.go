...
type LeasedRuntime struct {
	...
	executions        map[leasedExecutionKey]*activeLeasedExecution
}
...
func newLeasedRuntime(config LeasedRuntimeConfig, heartbeatDriver sessionLeaseHeartbeatDriver) (*LeasedRuntime, error) {
	...
	return &LeasedRuntime{
		...
		executions:        make(map[leasedExecutionKey]*activeLeasedExecution),
	}, nil
}
...
func (r *LeasedRuntime) runWithLease(ctx context.Context, ref session.SessionRef, execute func(context.Context) (agent.RunResult, error)) (agent.RunResult, error) {
	...
	execution := r.registerLeasedExecution(ref, lease.LeaseID, guard)
	result, err := execute(runCtx)
	if err != nil {
		cancel()
		finishErr := guard.finishAndErr()
		r.completeLeasedExecution(execution)
	...
	return r.wrapLiveHandle(result, ref, func(inner agent.Runner, onFinish func()) agent.Runner {
		wrapped := newLeasedRunner(inner, guard, cancel, func() {
			onFinish()
			r.completeLeasedExecution(execution)
		})
		execution.setRunner(baseLeasedRunner(wrapped))
		return wrapped
	}), nil
}
...
var (
	errSessionLeaseRenewalDeadline = errors.New("controlplane: session lease renewal did not complete before the expiry safety deadline")
	errSessionLeaseOwnershipLost   = fmt.Errorf("controlplane: session lease ownership was lost: %w", session.ErrLeaseConflict)
)
...
func (g *sessionLeaseGuard) setOnLoss(onLoss func()) {
	...
	alreadyLost := g.heartbeatErr != nil && onLoss != nil
	if alreadyLost {
		g.lossNotified = true
	}
	g.mu.Unlock()
...
func (g *sessionLeaseGuard) recordLoss(err error) {
	if g == nil || err == nil {
		return
	}
	g.mu.Lock()
...
func (g *sessionLeaseGuard) heartbeatOnce() error {
	...
	if err != nil {
		...
		if errors.Is(err, session.ErrLeaseConflict) {
			return errSessionLeaseOwnershipLost
		}
...
func reconcileSessionLeaseForRelease(
	...
	if durable.LeaseID != lease.LeaseID || durable.OwnerID != lease.OwnerID || durable.FencingToken != lease.FencingToken {
		// Another owner may legitimately acquire a newer fence after this
		// producer's lease expires. The stale owner must stop without touching
		// or reporting cleanup failure against the replacement lease.
		return session.SessionLease{}, false, nil
	}
...
func (r *leasedRunner) Events() iter.Seq2[*session.Event, error] {
	return func(yield func(*session.Event, error) bool) {
		...
		for event, err := range r.inner.Events() {
			if errors.Is(err, session.ErrLeaseConflict) {
				r.guard.recordLoss(errSessionLeaseOwnershipLost)
				break
			}
			if !yield(event, err) {
				completed = false
