...
func (c *Coordinator) handoffController(ctx context.Context, req agent.HandoffControllerRequest) (session.Session, error) {
	...
	if expected := strings.TrimSpace(req.ExpectedControllerEpoch); expected != "" &&
		strings.TrimSpace(activeSession.Controller.EpochID) != expected {
		return activeSession, fmt.Errorf(
			"controlplane: expected controller epoch %q, actual %q: %w",
			expected,
			activeSession.Controller.EpochID,
