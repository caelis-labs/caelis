...
func (r *Runtime) Start(ctx context.Context, req sandbox.CommandRequest) (sandbox.Session, error) {
	...
	if err != nil {
		cancel()
		return nil, fmt.Errorf("internal/adapters/sandbox/hostrunner: create stdin pipe: %w", err)
	}
	stdout, stdoutWriter, err := os.Pipe()
	if err != nil {
		_ = stdin.Close()
		cancel()
		return nil, fmt.Errorf("internal/adapters/sandbox/hostrunner: create stdout pipe: %w", err)
	}
	...
	if err != nil {
		...
		cancel()
		return nil, fmt.Errorf("internal/adapters/sandbox/hostrunner: create stderr pipe: %w", err)
	}
...
func (r *Runtime) OpenSession(id string) (sandbox.Session, error) {
	...
	if !ok {
		return nil, fmt.Errorf("internal/adapters/sandbox/hostrunner: session %q not found", id)
	}
	return session, nil
}
