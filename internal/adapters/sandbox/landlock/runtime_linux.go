...
func newRuntime(cfg Config) (sandbox.Runtime, error) {
	...
	return runnerruntime.New(runnerruntime.Config{
		...
		Status: sandbox.Status{
			RequestedBackend: sandbox.BackendLandlock,
			ResolvedBackend:  sandbox.BackendLandlock,
		},
...
func (l *landlockRunner) Run(ctx context.Context, req runnerruntime.Request) (sandbox.CommandResult, error) {
	runCtx := ctx
	cancel := func() {}
	if req.Timeout > 0 {
	...
	if err != nil {
		return sandbox.CommandResult{}, fmt.Errorf("tool: resolve landlock workdir failed: %w", err)
	}
	effectivePolicy := policy.Default(l.cfg, req.Constraints)
	...
	if err := cmd.Start(); err != nil {
		return sandbox.CommandResult{}, fmt.Errorf("tool: landlock sandbox command start failed: %w", err)
	}
...
func (l *landlockRunner) WaitSession(ctx context.Context, sessionID string, timeout time.Duration) (sandbox.CommandResult, error) {
	manager, err := l.asyncSessionManager()
	if err != nil {
	...
	if timeout > 0 {
		if _, err := manager.WaitSessionWithContextTimeout(ctx, sessionID, timeout); err != nil {
			if errors.Is(err, context.DeadlineExceeded) && ctx.Err() == nil {
				return sandbox.CommandResult{Route: sandbox.RouteSandbox, Backend: sandbox.BackendLandlock}, nil
			}
			return sandbox.CommandResult{}, err
		}
	...
	result.Backend = sandbox.BackendLandlock
	return result, err
}
...
func (l *landlockRunner) asyncSessionManager() (*cmdsession.SessionManager, error) {
	if l == nil || l.closed.Load() || l.sessionManager == nil {
		return nil, fmt.Errorf("internal/adapters/sandbox/landlock: runner is closed")
	}
...
func runInternalHelper(args []string) error {
	...
	needFSRestrictions := p.Type != policy.TypeDangerFull && p.Type != policy.TypeExternal
...
func landlockWritableRoots(p policy.Policy, workDir string) []string {
