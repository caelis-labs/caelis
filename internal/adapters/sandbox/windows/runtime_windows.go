...
import (
	...
	"github.com/OnslaughtSnail/caelis/internal/adapters/sandbox/windows/internal/capability"
	"github.com/OnslaughtSnail/caelis/internal/adapters/sandbox/windows/internal/job"
...
func (r *runtime) Start(ctx context.Context, req sandbox.CommandRequest) (sandbox.Session, error) {
	...
	if err != nil {
		_ = token.Close()
		cancel()
		return nil, fmt.Errorf("internal/adapters/sandbox/windows: create stdin pipe: %w", err)
	}
	...
	if err != nil {
		...
		_ = token.Close()
		cancel()
		return nil, fmt.Errorf("internal/adapters/sandbox/windows: create stdout pipe: %w", err)
	}
	...
	if err != nil {
		...
		_ = token.Close()
		cancel()
		return nil, fmt.Errorf("internal/adapters/sandbox/windows: create stderr pipe: %w", err)
	}
...
func (r *runtime) OpenSession(id string) (sandbox.Session, error) {
	...
	if !ok {
		return nil, fmt.Errorf("internal/adapters/sandbox/windows: session %q not found", id)
	}
	return session, nil
}
...
func (r *runtime) Reset(ctx context.Context) error {
	...
	if len(errs) > 0 {
		...
		return fmt.Errorf("internal/adapters/sandbox/windows: clean sandbox state: %w", joined)
	}
	sandbox.ReportPrepareProgress(ctx, sandbox.PrepareProgress{
		Phase:   "complete",
...
func (r *runtime) restrictedShellCommand(ctx context.Context, req sandbox.CommandRequest, interactive bool, policy workspacePolicy) (*exec.Cmd, win32.Token, error) {
	...
	if err != nil {
		return nil, 0, fmt.Errorf("internal/adapters/sandbox/windows: create restricted token: %w", err)
	}
...
func runCommandWithJob(ctx context.Context, cmd *exec.Cmd) error {
	if cmd == nil {
		return fmt.Errorf("internal/adapters/sandbox/windows: command is required")
	}
	if err := cmd.Start(); err != nil {
		return err
	}
	...
	case <-ctx.Done():
		...
		case <-time.After(windowsTerminateDrain):
			return errors.Join(
				ctx.Err(),
				fmt.Errorf("internal/adapters/sandbox/windows: command terminated before process wait completed"),
				terminateErr,
			)
		}
	}
...
func (r *runtime) policyForRequestWithBinding(req sandbox.CommandRequest, createSIDs bool) (workspacePolicy, error) {
	...
	if constraints.Permission != sandbox.PermissionWorkspaceWrite {
		return workspacePolicy{}, fmt.Errorf("internal/adapters/sandbox/windows: permission mode %q is unsupported by the Windows workspace-write sandbox", constraints.Permission)
	}
	...
	if len(writeRoots) == 0 {
		return workspacePolicy{}, fmt.Errorf("internal/adapters/sandbox/windows: at least one writable root is required")
	}
	...
	for _, root := range userWriteRoots {
		...
		for _, subpath := range r.cfg.ReadOnlySubpaths {
			...
			if _, err := os.Stat(path); err != nil {
				...
				return workspacePolicy{}, fmt.Errorf("internal/adapters/sandbox/windows: inspect deny-write path %s: %w", path, err)
			}
	...
	if err != nil {
		return workspacePolicy{}, fmt.Errorf("internal/adapters/sandbox/windows: bind write capability SIDs: %w", err)
	}
...
func existingWritableRoots(roots []string) ([]string, error) {
	...
	for _, root := range roots {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("internal/adapters/sandbox/windows: inspect writable root %s: %w", root, err)
		}
		if !info.IsDir() {
			return nil, fmt.Errorf("internal/adapters/sandbox/windows: writable root %s is not a directory", root)
		}
		out = append(out, root)
	}
	...
	if create {
		for _, dir := range sandboxEnvDirs(root) {
			if err := os.MkdirAll(dir, 0o700); err != nil {
				return "", fmt.Errorf("internal/adapters/sandbox/windows: prepare sandbox environment directory %s: %w", dir, err)
			}
		}
	}
	return root, nil
...
func (r *runtime) missingACLEntries(policy workspacePolicy) ([]acl.Entry, error) {
	...
	if envSID != "" {
		for _, path := range sandboxEnvDirs(policy.SandboxEnvRoot) {
			...
			if !info.IsDir() {
				return nil, fmt.Errorf("internal/adapters/sandbox/windows: sandbox environment path %s is not a directory", path)
			}
...
func (r *runtime) applyPolicyACLs(policy workspacePolicy) error {
	for _, root := range policy.WriteRoots {
		...
		if err != nil {
			if !os.IsNotExist(err) {
				return fmt.Errorf("internal/adapters/sandbox/windows: inspect writable root %s: %w", root, err)
			}
			continue
		}
		if !info.IsDir() {
			return fmt.Errorf("internal/adapters/sandbox/windows: writable root %s is not a directory", root)
		}
		if err := acl.ModifyFileDACL(root, allowEntries(policy.sidForWriteRoot(root))...); err != nil {
			return fmt.Errorf("internal/adapters/sandbox/windows: apply writable root ACL %s: %w", root, diagnoseACLWriteFailure(root, err))
		}
	}
	envSID := policy.sidForWriteRoot(policy.SandboxEnvRoot)
	if envSID != "" {
		for _, path := range sandboxEnvDirs(policy.SandboxEnvRoot) {
			...
			if err != nil {
				...
				return fmt.Errorf("internal/adapters/sandbox/windows: inspect sandbox environment path %s: %w", path, err)
			}
			if !info.IsDir() {
				return fmt.Errorf("internal/adapters/sandbox/windows: sandbox environment path %s is not a directory", path)
			}
			if err := acl.ModifyFileDACL(path, allowEntries(envSID)...); err != nil {
				return fmt.Errorf("internal/adapters/sandbox/windows: apply sandbox environment ACL %s: %w", path, diagnoseACLWriteFailure(path, err))
			}
		}
	}
	for _, path := range policy.DenyWritePaths {
		if _, err := os.Stat(path); err != nil {
			...
			return fmt.Errorf("internal/adapters/sandbox/windows: inspect deny-write path %s: %w", path, err)
		}
		if err := acl.ModifyFileDACL(path, denyEntries(policy.CapabilitySIDs)...); err != nil {
			return fmt.Errorf("internal/adapters/sandbox/windows: apply deny-write ACL %s: %w", path, diagnoseACLWriteFailure(path, err))
		}
	}
	return nil
}
...
func sandboxEnvironment(policy workspacePolicy, extra map[string]string) ([]string, error) {
	...
	if envRoot == "" {
		return nil, fmt.Errorf("internal/adapters/sandbox/windows: sandbox environment root is required")
	}
	...
	for _, dir := range sandboxEnvDirs(envRoot) {
		if err := os.MkdirAll(dir, 0o700); err != nil {
			return nil, fmt.Errorf("internal/adapters/sandbox/windows: prepare sandbox environment directory %s: %w", dir, err)
		}
	}
...
func writeSandboxPythonSiteCustomize(siteDir string) error {
	...
	if err := os.MkdirAll(siteDir, 0o700); err != nil {
		return fmt.Errorf("internal/adapters/sandbox/windows: prepare Python sandbox site directory %s: %w", siteDir, err)
	}
	path := filepath.Join(siteDir, "sitecustomize.py")
	if err := os.WriteFile(path, []byte(sandboxPythonSiteCustomize), 0o600); err != nil {
		return fmt.Errorf("internal/adapters/sandbox/windows: write Python sandbox sitecustomize %s: %w", path, err)
	}
	return nil
}
...
func resolveStateRoot(raw string) (string, error) {
	...
	home, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("internal/adapters/sandbox/windows: resolve user home: %w", err)
	}
