...
func (r *runtime) runElevatedRepair(ctx context.Context) error {
	...
	if err := os.MkdirAll(r.sandboxStateDir(), 0o700); err != nil {
		return fmt.Errorf("internal/adapters/sandbox/windows: prepare repair state directory: %w", err)
	}
	...
	if err := os.WriteFile(configFile, data, 0o600); err != nil {
		return fmt.Errorf("internal/adapters/sandbox/windows: write elevated repair request: %w", err)
	}
	exe, err := os.Executable()
	if err != nil {
		return fmt.Errorf("internal/adapters/sandbox/windows: resolve current executable: %w", err)
	}
	...
	if launchErr != nil {
		if resultErr == nil && strings.TrimSpace(result.Error) != "" {
			return fmt.Errorf("internal/adapters/sandbox/windows: elevated sandbox repair failed: %s", result.Error)
		}
		return fmt.Errorf("internal/adapters/sandbox/windows: launch elevated sandbox repair: %w", launchErr)
	}
	if resultErr != nil {
		return fmt.Errorf("internal/adapters/sandbox/windows: read elevated sandbox repair result: %w", resultErr)
	}
	if strings.TrimSpace(result.Error) != "" {
		return fmt.Errorf("internal/adapters/sandbox/windows: elevated sandbox repair failed: %s", result.Error)
	}
	if exitCode != 0 || !result.OK {
		return fmt.Errorf("internal/adapters/sandbox/windows: elevated sandbox repair exited with code %d", exitCode)
	}
	return nil
}
...
func (r *runtime) verifyRepairRootsWritable(ctx context.Context) error {
	...
	for _, root := range policy.WriteRoots {
		...
		if err := verifyDirectoryFileWrite(root); err != nil {
			return fmt.Errorf("internal/adapters/sandbox/windows: elevated repair refuses root without current-user file write access %s: %w", root, err)
		}
	}
	return nil
}
...
func runInternalRepairFromConfig(configFile string) error {
	...
	if !ok {
		return fmt.Errorf("internal/adapters/sandbox/windows: unexpected repair runtime type %T", rt)
	}
...
func validateElevatedRepairConfig(cfg sandbox.Config) error {
	if normalizeBackendAlias(cfg.RequestedBackend) != sandbox.BackendWindows {
		return fmt.Errorf("internal/adapters/sandbox/windows: elevated repair only supports the Windows sandbox backend")
	}
	...
	if workspaceRoot == "" {
		return fmt.Errorf("internal/adapters/sandbox/windows: workspace cwd is required for elevated repair")
	}
	...
	for _, subpath := range cfg.ReadOnlySubpaths {
		if !safeRelativeSubpath(subpath) {
			return fmt.Errorf("internal/adapters/sandbox/windows: elevated repair refuses unsafe read-only subpath: %s", subpath)
		}
	}
	return nil
}
...
func validateRepairDirectory(label string, path string) error {
	...
	if path == "" {
		return fmt.Errorf("internal/adapters/sandbox/windows: %s path is required", label)
	}
	info, err := os.Stat(path)
	if err != nil {
		return fmt.Errorf("internal/adapters/sandbox/windows: inspect %s path %s: %w", label, path, err)
	}
	if !info.IsDir() {
		return fmt.Errorf("internal/adapters/sandbox/windows: %s path %s is not a directory", label, path)
	}
	if isUNCPath(path) || isVolumeRoot(path) || isKnownSystemPath(path) {
		return fmt.Errorf("internal/adapters/sandbox/windows: elevated repair refuses unsafe %s path: %s", label, path)
	}
	reparse, err := isReparsePoint(path)
	if err != nil {
		return fmt.Errorf("internal/adapters/sandbox/windows: inspect %s reparse point %s: %w", label, path, err)
	}
	if reparse {
		return fmt.Errorf("internal/adapters/sandbox/windows: elevated repair refuses reparse-point %s path: %s", label, path)
	}
	return nil
}
