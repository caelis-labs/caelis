...
func TestGatewayDoesNotExposeApproverAsReviewerAndReleasesClosedSessionContext(t *testing.T) {
	t.Parallel()

	releaser := &approvalContextReleaseRecorder{}
	gw, err := New(Config{
		Sessions:         staticSessionService{session: session.Session{}},
		Runtime:          mockRuntime{},
		Resolver:         staticResolver{},
		ApprovalApprover: releaser,
	})
	if err != nil {
		t.Fatal(err)
	}
	if gw.approvalReviewer != nil {
		t.Fatalf("approvalReviewer = %T, want no Approver-to-Reviewer reverse adapter", gw.approvalReviewer)
	}
	ref := session.SessionRef{SessionID: "closed-main-session"}
	gw.CloseSessionApprovals(ref, "session_closed")
	if len(releaser.released) != 1 || releaser.released[0].SessionID != ref.SessionID {
		t.Fatalf("released refs = %#v, want %q", releaser.released, ref.SessionID)
	}
}

type approvalContextReleaseRecorder struct {
	released []session.SessionRef
}

func (*approvalContextReleaseRecorder) Decide(context.Context, ApprovalReviewRequest) (ApprovalReviewResult, error) {
	return ApprovalReviewResult{}, nil
}

func (r *approvalContextReleaseRecorder) ReleaseApprovalContext(ref session.SessionRef) {
...
func TestBeginTurnRecoversLostLocalProducerBeforeAdmittingNextRequest(t *testing.T) {
	t.Parallel()

	activeSession := session.Session{SessionRef: session.SessionRef{
		AppName: "caelis", UserID: "u", SessionID: "s-recover-lost", WorkspaceKey: "ws",
	}}
	runtime := newLostRunRecoveryRuntime(activeSession)
	gw, err := New(Config{
		Sessions: staticSessionService{session: activeSession},
		Runtime:  runtime,
		Resolver: staticResolver{resolved: ResolvedTurn{}},
	...
	firstRunner := <-runtime.started
	defer first.Handle.Close()

	second, err := gw.BeginTurn(context.Background(), BeginTurnRequest{
		SessionRef: activeSession.SessionRef,
		Input:      "first request after wake",
	})
	if err != nil {
		t.Fatalf("BeginTurn(second) error = %v", err)
	}
	secondRunner := <-runtime.started
	defer second.Handle.Close()
	defer second.Handle.Cancel()

	if firstRunner.cancelCalls() != 1 {
		t.Fatalf("lost runner cancel calls = %d, want 1", firstRunner.cancelCalls())
	}
	if runtime.recoveryCalls() != 1 {
		t.Fatalf("lost-run recovery calls = %d, want 1", runtime.recoveryCalls())
	}
	if secondRunner == firstRunner {
		t.Fatal("second request reused the lost producer")
	}
}
...
func TestResolveApprovalRequestSerializesDifferentModelSteps(t *testing.T) {
	t.Parallel()
	...
	release := make(chan struct{})
	approver := &blockingApprovalApprover{
	...
	stepRefs := map[string][]*tool.ModelStepRef{
		"step-1": tool.NewConcurrentModelStepRefs("step-1", 2),
		"step-2": tool.NewConcurrentModelStepRefs("step-2", 2),
	}
	...
	go func() {
		resp, err := gw.resolveApprovalRequest(context.Background(), context.Background(), handle, request("first", "step-1"), nil)
...
func TestResolveApprovalRequestRunsSameModelStepAutoReviewsConcurrently(t *testing.T) {
	t.Parallel()

	activeSession := session.Session{SessionRef: session.SessionRef{
		AppName: "caelis", UserID: "u", SessionID: "same-step-auto-review", WorkspaceKey: "ws",
	}}
	release := make(chan struct{})
	approver := &blockingApprovalApprover{started: make(chan string, 2), release: release}
	gw, err := New(Config{
		Sessions: staticSessionService{
			session: activeSession,
			state:   map[string]any{StateCurrentApprovalMode: string(ApprovalModeAutoReview)},
	...
	handle := newTestTurnHandle()
	stepRefs := tool.NewConcurrentModelStepRefs("shared-step", 2)
	request := func(callID string, index int) *agent.ApprovalRequest {
		return &agent.ApprovalRequest{
			SessionRef: activeSession.SessionRef,
			Session:    activeSession,
			RunID:      "run-1",
	...
	results := make(chan result, 2)
	go func() {
		resp, runErr := gw.resolveApprovalRequest(context.Background(), context.Background(), handle, request("first", 0), nil)
		results <- result{callID: "first", resp: resp, err: runErr}
	}()
	select {
	case callID := <-approver.started:
		if callID != "first" {
			t.Fatalf("first auto-review call = %q", callID)
		}
	case <-time.After(time.Second):
		t.Fatal("first same-step auto-review did not start")
	}

	go func() {
		resp, runErr := gw.resolveApprovalRequest(context.Background(), context.Background(), handle, request("second", 1), nil)
		results <- result{callID: "second", resp: resp, err: runErr}
	}()
	waitForApprovalQueueLength(t, handle, 2)
	select {
	case callID := <-approver.started:
		if callID != "second" {
			t.Fatalf("second concurrent auto-review call = %q", callID)
		}
	case <-time.After(time.Second):
		t.Fatal("second same-step auto-review remained serialized")
	}
	for _, pending := range handle.approvals.queueSnapshot() {
		select {
		case <-pending.activated:
		default:
			t.Fatalf("same-step approval %q was not activated with its cohort", pending.id)
		}
	}
	...
	for range 2 {
		...
		case got := <-results:
			if got.err != nil || !got.resp.Approved {
				t.Fatalf("auto-review result for %s = %+v, %v", got.callID, got.resp, got.err)
			}
		case <-time.After(time.Second):
			t.Fatal("same-step auto-review did not resolve")
		}
	}
}
...
func TestBeginTurnSessionApprovalModeOverridesDefaultManual(t *testing.T) {
	...
	_ = collectHandleEvents(t, result.Handle)
	got := recorder.snapshot()
	if got := rt.executionCount(); got != 1 {
		t.Fatalf("approved executor calls = %d, want 1", got)
	}
	for _, env := range got {
...
func TestPersistApprovalReviewSessionAccountingUsesSessionStateNotHistory(t *testing.T) {
	...
	sessions := &revisionConflictOnceSessionService{Service: inmemory.NewStore(inmemory.Config{})}
	...
	if got := sessions.updateCalls.Load(); got != 2 {
		t.Fatalf("UpdateState() calls = %d, want one revision-conflict retry", got)
	}
	...
	got := session.UsageSnapshotFromMapForProvider(anyMapValue(accounting["auto_review"]), "")
	...
	modelUsage := session.UsageSnapshotFromMapForProvider(anyMapValue(row["usage"]), "")
...
func TestBeginTurnAutoReviewReturnsAvailabilityWithoutSyntheticApproval(t *testing.T) {
	...
	reviewer := &scriptedApprovalReviewer{steps: []scriptedApprovalReviewStep{
		{err: errors.New("guardian execution failed")},
		{result: ApprovalReviewResult{Approved: true}},
	}}
	rt := &approvalRuntime{session: activeSession, requests: 4}
	gw, err := New(Config{
		Sessions:         staticSessionService{session: activeSession},
		Runtime:          rt,
		Resolver:         staticResolver{resolved: ResolvedTurn{RunRequest: agent.RunRequest{}}},
	...
	events := recorder.snapshot()
	if got := rt.executionCount(); got != 0 {
		t.Fatalf("failed Guardian executor calls = %d, want 0", got)
	}
	if got := reviewer.callCount(); got != 1 {
		t.Fatalf("Guardian review calls = %d, want immediate stop after 1", got)
	}
	failedReviews := 0
	for _, env := range events {
		if env.Kind == eventstream.KindApprovalReview && env.ApprovalReview != nil {
			if env.ApprovalReview.Status == string(ApprovalReviewStatusFailed) {
				failedReviews++
			}
		}
	}
	if failedReviews != 1 {
		t.Fatalf("failed Guardian reviews = %d, want 1: %#v", failedReviews, events)
	}
	var terminalError eventstream.Envelope
	for _, event := range events {
		if event.Kind == eventstream.KindError {
			terminalError = event
		}
	}
	if terminalError.Kind != eventstream.KindError {
		t.Fatalf("events = %#v, want guardian_unavailable error", events)
	}
	var gatewayErr *Error
...
func TestResolveActiveAutoApprovalPropagatesApprovalDeadlineWithoutSyntheticDecision(t *testing.T) {
	t.Parallel()
	...
	if !errors.Is(err, context.DeadlineExceeded) {
		t.Fatalf("resolveActiveAutoApproval() error = %v, want approval deadline", err)
	}
	if got := reviewer.callCount(); got != 1 {
