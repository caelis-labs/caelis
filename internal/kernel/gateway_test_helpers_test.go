...
type revisionConflictOnceSessionService struct {
	session.Service
	conflicted  atomic.Bool
	updateCalls atomic.Int64
}

func (s *revisionConflictOnceSessionService) UpdateState(ctx context.Context, req session.UpdateStateRequest) (session.Session, error) {
	s.updateCalls.Add(1)
	if s.conflicted.CompareAndSwap(false, true) {
		...
		return session.Session{}, &session.RevisionConflictError{
...
type blockingApprovalApprover struct {
	started chan string
	release <-chan struct{}
}

func (a *blockingApprovalApprover) Decide(ctx context.Context, req ApprovalReviewRequest) (ApprovalReviewResult, error) {
...
func waitForRunnerSubmissions(t *testing.T, runner *submitRecordingBlockingRunner, want int) {
	t.Helper()
	...
	defer tick.Stop()
	for {
		got := len(runner.snapshot())
		if got == want {
			return
		}
		select {
		case <-deadline.C:
			t.Fatalf("runner submissions = %d, want %d", got, want)
		case <-tick.C:
		}
	}
}
...
type lostRunRecoveryRuntime struct {
	...
	mu           sync.Mutex
	active       *lostRunRecoveryRunner
	recoveries   int
	nextRunnerID int
}

func newLostRunRecoveryRuntime(activeSession session.Session) *lostRunRecoveryRuntime {
	return &lostRunRecoveryRuntime{
		session: activeSession,
		started: make(chan *lostRunRecoveryRunner, 2),
	}
}

func (r *lostRunRecoveryRuntime) Run(context.Context, agent.RunRequest) (agent.RunResult, error) {
	r.mu.Lock()
	r.nextRunnerID++
	runner := newLostRunRecoveryRunner(fmt.Sprintf("run-recovery-%d", r.nextRunnerID))
	r.active = runner
	r.mu.Unlock()
	r.started <- runner
	return agent.RunResult{Session: r.session, Handle: runner}, nil
}

func (*lostRunRecoveryRuntime) RunState(context.Context, session.SessionRef) (agent.RunState, error) {
	return agent.RunState{}, nil
}

func (r *lostRunRecoveryRuntime) RecoverLostRun(context.Context, session.SessionRef) (bool, error) {
	r.mu.Lock()
	r.recoveries++
	runner := r.active
	r.mu.Unlock()
...
func (r *lostRunRecoveryRuntime) recoveryCalls() int {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.recoveries
}

type lostRunRecoveryRunner struct {
	id       string
	done     chan struct{}
	doneOnce sync.Once
...
func newLostRunRecoveryRunner(id string) *lostRunRecoveryRunner {
	return &lostRunRecoveryRunner{id: id, done: make(chan struct{})}
}
...
func (r *lostRunRecoveryRunner) Cancel() agent.CancelResult {
	r.mu.Lock()
	r.cancels++
	r.mu.Unlock()
...
func (r *lostRunRecoveryRunner) Close() error {
	r.doneOnce.Do(func() { close(r.done) })
	return nil
}

func (r *lostRunRecoveryRunner) cancelCalls() int {
	r.mu.Lock()
	defer r.mu.Unlock()
	return r.cancels
}
