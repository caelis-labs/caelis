...
func TestSessionApprovalCoordinatorKeepsConcurrentStepOpenForLateSibling(t *testing.T) {
	t.Parallel()

	ref := session.SessionRef{SessionID: "session-late-step-approval"}
	coordinator := newApprovalCoordinator(ref)
	handle := newTurnHandle(turnHandleConfig{handleID: "turn-a", sessionRef: ref, approvals: coordinator})
	stepRefs := tool.NewConcurrentModelStepRefs("step-a", 2)
	request := func(token string, step *tool.ModelStepRef) *agent.ApprovalRequest {
	...
	if err := coordinator.resolve(first, ApprovalDecision{Approved: true}); err != nil {
		t.Fatal(err)
	}
	select {
	case <-other.activated:
		t.Fatal("different approval activated before the concurrent step finished admission")
	default:
	}
	...
	case <-late.activated:
	default:
		t.Fatal("late same-step sibling did not rejoin its active cohort")
	}
	select {
	case <-other.activated:
		t.Fatal("interleaved approval overtook a late same-step sibling")
	default:
	}
	if err := coordinator.resolve(late, ApprovalDecision{Approved: true}); err != nil {
		t.Fatal(err)
	}
	select {
	case <-other.activated:
	default:
		t.Fatal("different approval did not activate after the concurrent step completed")
	}
	coordinator.release(other, "abandoned")
}

func TestSessionApprovalCoordinatorAbandonReleasesOpenConcurrentStep(t *testing.T) {
	t.Parallel()

	ref := session.SessionRef{SessionID: "session-abandoned-step-approval"}
	coordinator := newApprovalCoordinator(ref)
	firstOwner := newTurnHandle(turnHandleConfig{handleID: "turn-a", sessionRef: ref, approvals: coordinator})
	...
	select {
	case <-other.activated:
		t.Fatal("different owner activated while the concurrent step remained open")
	default:
	}
	coordinator.abandonOwner(firstOwner, "cancelled")
	select {
	case <-first.done:
	default:
		t.Fatal("abandoned concurrent approval was not released")
	}
	select {
	case <-other.activated:
	default:
		t.Fatal("different owner did not activate after the concurrent step was abandoned")
	}
	coordinator.release(other, "abandoned")
}
