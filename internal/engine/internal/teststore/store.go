// Package teststore provides a minimal in-memory session store for engine
// tests without importing concrete adapters.
package teststore
...
func (s *Store) Create(ctx context.Context, req session.StartRequest) (session.Session, error) {
	...
	s.sessions[id] = &record{session: active, state: session.State{}}
...
func (s *Store) Append(ctx context.Context, ref session.Ref, events []session.Event) (session.Cursor, error) {
	...
	for _, event := range events {
		...
		if strings.TrimSpace(next.SessionID) == "" {
			next.SessionID = rec.session.SessionID
		}
		if next.Time.IsZero() {
...
func (s *Store) Events(ctx context.Context, query session.EventQuery) (session.EventPage, error) {
	...
	s.mu.RLock()
	defer s.mu.RUnlock()
	rec, ok := s.sessions[strings.TrimSpace(query.Ref.SessionID)]
	if !ok {
