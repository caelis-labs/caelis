...
TUI_GOLDEN_SELECTOR ?= ^(TestTeamCellFocusRenderedFrames|TestRegressionACPEventstreamToolCallFrame120x32|TestBotGoldenFrames|TestBotModelPickerGoldenFrames|TestBotPickerLongSelectorGoldenFrames)$$
TUI_INTERACTION_SELECTOR ?= ^(TestTeamCellFocusLightTheme|TestTeamCellFocusKeyboardCyclesFieldsAndSkipsUnsupported|TestTeamCellFocusSingleClickEditsControlsAndAppliesModel|TestTeamCellFocusMemoryVerifierSharesStewardAndOffersJev|TestTeamCellFocusMainQuickControlsUseTypedBinding|TestTeamGuardianHasOneRowAndConditionalAuxiliarySelector|TestTeamGuardianSelectorMouseGeometryAndNarrowRendering|TestTeamGuardianDisconnectHidesAuxiliaryWithoutChangingAgent|TestRegressionACPEventstreamWhitespaceOnlyAssistantChunkDoesNotRenderBeforeTool|TestHandleACPEventEnvelopeRendersSemanticSpawnEventsOnce|TestHandleACPEventEnvelopeScopedChildTerminalKeepsOneSpawnPanelAndMainTurnAlive|TestRegressionSubagentInputKeepsItsPlaceWithinTurn|TestRegressionTaskMailboxCatchesUpInBoundedBatches|TestTaskMailboxKeepsReplacementAndActivityBoundaries|TestShortAndLongSpawnRowsOpenOverlayInsteadOfFolding|TestSendMessageToChildOpensOverlayInsteadOfFolding|TestLaterTurnAgentCommunicationOpensRetainedOwnerOverlay|TestUnresolvedAndParentTargetsDoNotCreateOverlayLinks|TestSessionPickerOpensWhileTurnRunsAndMarksRunningRows|TestSessionPickerEscapeOnlyClosesAndDoesNotInterrupt|TestSessionPickerKeyboardSelectionAttachesSelectedSession|TestSessionPickerMouseSelectionAttachesSelectedSession|TestSessionPickerNarrowCJKRowsStayInPhysicalFrame|TestBotSelectionPublishesOnlyAfterAttach|TestBotRepeatedSelectionKeepsAcceptedTarget|TestBotModelSelectionUsesCanonicalIdentity|TestBotFailedAttachKeepsPreviousBot|TestBotPickerLongSelectorRowsStayReadableAndHitTestable|TestBotPickerRowLineStaysWithinColumns|TestBotConfigurationRefreshUsesFocusedRead|TestBotConfigurationRefreshRejectsStaleSnapshots|TestBotLocalSaveResultCannotRollBackRefreshedConfiguration|TestBotLiveTerminalStatus|TestBotProvisionalWorkPreservesReplyStatus|TestBotCanonicalRunningSupersedesUnknown|TestBotLiveTerminalRejectsOtherScopeSessionAndGeneration|TestBotTerminalOwnerRequiresActiveConversation|TestBotReconnectStatusMatchesLiveTerminal|TestBotAllowlistRefusesNonBotCommands|TestBotCommandRefreshPreservesSurfaceCatalog|TestBotSlashCompletionDoesNotLoadWorkspaceSkills|TestBotSharedConfigurationCommandsAndAliases|TestRunBotCreateFlowOptionalDescription|TestBotCreateDescriptionKeyboard|TestRunBotCreateFlowCancelledDoesNotCreate|TestBotImageOnlyInputBlockedBeforeBootstrap|TestBotSettingsResultDoesNotOverrideSwitchedBot|TestBotRecoveredUnknownOutcomeShowsInChrome)$$
CONTROL_TURN_REGRESSION_SELECTOR ?= ^(TestSessionTurnClientAttachesBeforePromptAndFiltersExactTarget|TestFeedBrokerSpoolDeliversWithoutProducerConsumerCoupling|TestSessionTurnClientRoutesApprovalAndCancelWithoutClosingSession|TestFeedBrokerGapFillsBeforeOutOfOrderDurablePublish|TestFeedBrokerGapFillKeepsLiveContentOwnership|TestFeedBrokerCatchupSeparatesNarrativeIdentityAndContent|TestFeedBrokerSuppressedFinalAdvancesCanonicalBoundary|TestSessionTurnWithoutSpoolCorrelatesCanonicalTargetByDurableTurnID|TestFeedCanonicalReplacementSupportsLongSession|TestSessionTurnCanonicalFallbackRejectsConflictingIdentity|TestFeedBrokerWithoutSpoolErrorsWhenTerminalOvertakesMissingCanonicalEvent|TestProcessRestartRebuildsDurableClientStateFromSessionTruth)$$
...
PRODUCT_TUI_SELECTOR ?= ^(TestProductScenarioContextCompactionRuntimeToPhysicalTUI|TestProductSessionObservationTracksRealHostTurns|TestProductUIPreferencesSurviveHostRestart|TestProductBotSelectionAndInputRouteThroughRealHost|TestProductBotAdapterRejectsImplicitSessionCreation|TestProductBotCatalogReadsUseSelectedConversationAddress|TestProductOrdinaryResumeCatalogReadsUseSelectedSessionAddress|TestProductBotModelSelectionPreservesConfiguration|TestProductBotCommandCatalogSurvivesHostRefresh|TestProductBotTeamOverlayReadsAndSavesHostConfiguration|TestProductBotCreateWithDescription|TestProductBotObservesOtherWindowConfiguration|TestProductBotLiveAndReattachedTerminalStatus)$$
PRODUCT_RUNTIME_SELECTOR ?= ^(TestRuntimeRecoveryConvergesInterruptedCommand|TestRuntimeRecoveryCommandRepairFileStoreRoundTrip|TestSessionWriteQueueCancellationPassesAdmissionToSuccessor|TestSessionWriteQueueCancellationUnlinksBeforePredecessorCompletes|TestTwoRuntimesRejectStaleCompactionAndRebuildWholeModelContext|TestRuntimeAutoCompactFailurePublishesLiveNotice|TestTaskCancelRejectsSubagentWithoutRemoteEffect|TestSubagentCompletionNoticeUsesInterruptionLanguage|TestSubmitChildInputBindsCanonicalParentSourceNotLocalAgentName|TestRuntimeSendMessageToChildUsesParentSourceNotLocal|TestRuntimeSendMessageDoesNotTreatLocalAsParentTarget|TestSubmitChildInputRejectsReservedParentSourceAlias|TestSubmitChildInputRejectsStaleControllerAfterHandoff)$$
...
PRODUCT_BOT_SELECTOR ?= ^(TestBotNotebookToolEffectsPrefixAndContextRoundTrip|TestBotNotebookIndexDiscoverableAfterWatermarkCompaction|TestBotLegacyNotebookExplicitEnableBoundary|TestBotLegacyNotebookProvisionFailurePreservesAdmission|TestBotLegacyNotebookEnableRejectsStaleAndActiveTurn|TestBotRuntimeIsolatedCapabilitiesAndStableCanonicalPrefix|TestBotRuntimeConcurrentInputCancelAndSettings|TestBotIdentityIsolationPersistenceAndLifecycle|TestBotCreationUnknownReceiptRecoversWithoutDuplicateConversation|TestBotCommittedSaveKeepsNewModelPinWhenClientCancels|TestDormantSessionRuntimeStateRecoversCanonicalOutcomeWithoutActivation)$$
...
product-acceptance: cache-dirs
	...
	GO_TEST_TIMEOUT=$(GO_TEST_TIMEOUT) "$(BASH)" ./scripts/go_test_nonempty.sh ./app/gatewayapp '$(PRODUCT_BOT_SELECTOR)' product-bot
