...
func TruncateResultWithInfo(result Result, policy TruncationPolicy) (Result, TruncationInfo) {
	...
	out.Content = sdkmodel.CloneParts(result.Content)
	out.Meta = maps.Clone(result.Meta)
	if len(out.Content) == 0 {
		payload, info := TruncateMap(out.Meta, policy)
		out.Content = []sdkmodel.Part{sdkmodel.NewJSONPart(mustMarshalMap(payload))}
		return out, info
	}
...
func TruncateParts(parts []sdkmodel.Part, policy TruncationPolicy) ([]sdkmodel.Part, TruncationInfo) {
	info := newTruncationInfo(policy)
	budgetTokens := policy.tokenBudget()
	if budgetTokens <= 0 {
		return sdkmodel.CloneParts(parts), info
	}
	...
	out := make([]sdkmodel.Part, 0, len(parts)+1)
	omitted := 0
	for _, part := range sdkmodel.CloneParts(parts) {
		switch {
		case part.JSON != nil:
			if remaining <= 0 {
				omitted++
	...
	if omitted > 0 {
		out = append(out, sdkmodel.NewTextPart(fmt.Sprintf("[omitted %d tool result content items]", omitted)))
	}
	info.Truncated = true
...
func estimateTokensForParts(parts []sdkmodel.Part) int {
	sum := 0
