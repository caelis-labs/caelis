...
func TestACPAgentNormalizesExternalUpdatesIntoInvocationSessionEvents(t *testing.T) {
	...
	factory.client.promptFn = func(context.Context, string, string, map[string]any) (client.PromptResponse, error) {
		...
		return client.PromptResponse{StopReason: "end_turn"}, nil
	}
...
func TestACPAgentBridgesPermissionRequestsToApprovalRequester(t *testing.T) {
	factory := &fakeACPClientFactory{}
	factory.client.promptFn = func(ctx context.Context, _ string, _ string, _ map[string]any) (client.PromptResponse, error) {
		resp, err := factory.callbacks.OnPermissionRequest(ctx, client.RequestPermissionRequest{
			SessionID: "remote-session",
...
func TestACPAgentSendsRemoteCancelWhenInvocationContextCancels(t *testing.T) {
	...
	factory.client.promptFn = func(ctx context.Context, _ string, _ string, _ map[string]any) (client.PromptResponse, error) {
		close(promptStarted)
		<-ctx.Done()
		return client.PromptResponse{}, ctx.Err()
	}
...
func TestACPAgentContinuationLoadsExistingRemoteSession(t *testing.T) {
	finalTrue := true
	factory := &fakeACPClientFactory{}
	factory.client.promptFn = func(_ context.Context, sessionID string, prompt string, _ map[string]any) (client.PromptResponse, error) {
		factory.callbacks.OnUpdate(client.UpdateEnvelope{
			SessionID: sessionID,
			Update: client.ContentChunk{
				SessionUpdate: client.UpdateAgentMessage,
...
type fakeACPClientFactory struct {
	...
	permissionResponse client.RequestPermissionResponse
}

func (f *fakeACPClientFactory) Start(_ context.Context, callbacks ACPClientCallbacks) (ACPClient, error) {
...
type fakeACPClient struct {
	mu              sync.Mutex
	promptFn        func(context.Context, string, string, map[string]any) (client.PromptResponse, error)
...
func (c *fakeACPClient) Initialize(context.Context) (client.InitializeResponse, error) {
	return client.InitializeResponse{}, nil
}

func (c *fakeACPClient) NewSession(context.Context, string, map[string]any) (client.NewSessionResponse, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.newSessionCalls++
	return client.NewSessionResponse{SessionID: "remote-session"}, nil
}

func (c *fakeACPClient) LoadSession(_ context.Context, sessionID string, _ string, _ map[string]any) (client.LoadSessionResponse, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.loadedSessions = append(c.loadedSessions, sessionID)
	return client.LoadSessionResponse{}, nil
}

func (c *fakeACPClient) Prompt(ctx context.Context, sessionID string, text string, meta map[string]any) (client.PromptResponse, error) {
	c.mu.Lock()
	c.promptSessionID = sessionID
	c.promptText = text
	c.mu.Unlock()
	if c.promptFn != nil {
		return c.promptFn(ctx, sessionID, text, meta)
	}
	return client.PromptResponse{StopReason: "end_turn"}, nil
}
...
func (c *fakeACPClient) Close(context.Context) error { return nil }
