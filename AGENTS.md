...
- `gatewayapp.Stack` must own Runtime composition through a named private field. Do not anonymously embed `runtimeComposition` into `Stack` or export the composition's fields; cross the Host boundary only through deliberate focused service getters. Do not recreate a wide function bag such as the retired `ControlRuntimeView`, or add direct Stack mirrors for execution, configuration revision, or methods already owned by Model, Agent, Status, Runtime, workspace, preparation, or message-delivery services.
