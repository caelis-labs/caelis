...
- Standard ACP wire contracts and connections come from `acp-go-sdk`. Do not add a new standalone ownership domain under root `protocol/acp/*`; its remaining schema, semantic, projection, eventstream, and metadata packages are transitional and must converge by function into Control, presentation, reusable SDK, or Host-private adapter owners. Product wire types must not flow inward.
