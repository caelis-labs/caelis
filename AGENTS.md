...
- `app/gatewayapp/controladapter` is Host-private server assembly, not a second product API. Its root package must not depend on the concrete `gatewayapp.Stack`; only `local` consumes it directly, translates Host views, and binds principal-sensitive capabilities. Other production packages use `control/appserver` or focused Control contracts.
