...
- `ports/controlclient` is the transport-neutral product client contract. `internal/controlclient` owns authorization, command idempotency, Session feeds, replay, and state assembly; `surfaces/appserver` only maps HTTP/SSE and authentication. Do not add new client operations to transitional `protocol/acp/control.Service` or Surface-private APIs.
