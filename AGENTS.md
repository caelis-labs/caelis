...
- Prefer public contracts in `ports/*`; keep private glue in `internal/*`; avoid mixing app-control contracts with reusable runtime contracts.
...
- Before committing, run `make commit-check`; it includes formatting, `golangci-lint`, `arch-lint`, vet, tests, and build.
