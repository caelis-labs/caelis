...
| ID | Status | Required closing evidence | Current evidence / next action |
| --- | --- | --- | --- |
| P1-1 ACP contract and Control ownership | in progress | One SDK semantic DTO owner; product wire only encodes/projects; product assembly/profile/process/UI/selection/handoff commit moved to Control; built-in/external conformance suite | Product assembly is now root-private `internal/controlassembly`; SDK Runtime no longer owns product mode/config state keys, applies product overrides, validates an Agent registry, or refreshes product Agent configs. UI-oriented controller status, handoff coordination, and explicit wire/semantic conformance remain. |
