...
| ID | Status | Required closing evidence | Current evidence / next action |
| --- | --- | --- | --- |
| P1-1 ACP contract and Control ownership | in progress | One SDK semantic DTO owner; product wire only encodes/projects; product assembly/profile/process/UI/selection/handoff commit moved to Control; built-in/external conformance suite | Product assembly is root-private `internal/controlassembly`; SDK Runtime no longer owns product mode/config state keys, applies product overrides, validates an Agent registry, or refreshes product Agent configs. Remote command/config/model/mode status and mutation are product ACP-manager/gateway contracts. `agent-sdk/session` is now the documented normalized ACP semantic owner; `protocol/acp/semantic` is the one wire codec used by external ingress and built-in projection, with JSON-round-trip conformance across message/tool/plan updates. Handoff commit and context-routing coordination remain. |
