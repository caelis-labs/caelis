...
这一阶段的验收标准不是“少了多少行”，而是这些体验必须完全保留：ACP transcript 渲染、RUN_COMMAND/SPAWN/TASK 面板、approval 状态、reasoning folding、tool output truncation、dark/light theme、自适应宽度、markdown/glamour 渲染、slash command completion。建议配套 golden/snapshot 测试锁定输出。
...
第五阶段处理 local runtime，特别是 `impl/agent/local/task_runtime.go`。这个文件约 2,918 行，里面混着 RUN_COMMAND、TASK、SPAWN/subagent、持久化、stream、approval、rehydration、snapshot、stdin write、cancel/wait 等逻辑。它是功能密集区，不能粗暴删，但非常适合抽象状态机。
...
这样 `TASK wait/write/cancel` 不需要到处判断 target kind。状态映射、tool result payload、stream append、persist entry 都可以共享。用户层面的 `RUN_COMMAND`、`SPAWN`、`TASK wait`、`TASK write`、`TASK cancel` 行为不变，但代码会更清晰。
...
7. **PR-7：task runtime 状态机化**
   把 RUN_COMMAND/SPAWN/subagent/TASK 生命周期统一到 `TaskTarget`。
