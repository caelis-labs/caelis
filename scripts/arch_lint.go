...
func boundaryRule(rel string, importPath string, modulePath string) string {
	if importPath == "testing" && !strings.HasSuffix(rel, "_test.go") && !isProductionTestSupportPackage(rel) {
		return "production Go files must not import testing outside explicit test-support packages"
	}
	...
	case strings.HasPrefix(rel, "surfaces/"):
		...
		if target == "internal/kernel" || strings.HasPrefix(target, "internal/kernel/") ||
			target == "agent-sdk/runtime" || strings.HasPrefix(target, "agent-sdk/runtime/") {
			return "surfaces must use Control clients and projected Envelopes, not Runtime or Kernel"
		}
...
func isProductionTestSupportPackage(rel string) bool {
